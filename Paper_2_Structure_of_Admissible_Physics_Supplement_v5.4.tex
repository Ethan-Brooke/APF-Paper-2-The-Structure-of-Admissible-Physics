% v5.4 (2026-07-15): REVIEW 5.3.01 ACCEPT SUBJECT TO MINOR REVISION.  All manuscript-side items implemented:
%   .01 Theorem A made self-contained by adding a general matter action rho_M, its integral character matrix C, and the physical
%   quotient T_phys = T_rel/ker(rho_M); separation deficiency now supplies necessity, while a specified anomaly-compatible separating
%   character supplies sufficiency; the generic conclusion is minimum EFFECTIVE RANK, not a unique ray, and the two-ray counterexample
%   C={(0,0),(1,0),(0,1)} is owned explicitly; directional uniqueness remains only in the SM-specialized Theorem B.
%   .02 H3's character condition typed correctly: rows c(r) assemble into C in Z^{5x2}, a cocharacter q produces y(q)=Cq, and the
%   all-left-handed hypercharge ray lies in im(C_Q:Q^2->Q^M); every proof reference to the former ill-typed ``image of c'' is replaced.
%   .03 P1 completed for ALL SU(3) irreps by the (p,q) dimension/Casimir/index formulas, the complete dim<10 list, and the proof that
%   min T among dim>=10 is 15/2 at 10 and 10bar; .04 T_field's finite-ansatz scope moved into the theorem statement itself.
%   .05 Theorem B and L_hypercharge now repeat the right-handed-partner versus all-left-handed sign convention at the conclusion.
%   .06 C10 removed from the fermion argmin ledger (now C1--C9): the equal-unit Weyl/Higgs/gauge convention is named only where it is
%   load-bearing, in L_count and the 61-channel ledger; all current-text C1--C9 references updated.
%   .07 Code--theorem concordance language tempered: executable checks certify programmed finite arithmetic/predicates, not physical
%   premises; the current proof-scope engine SHA ff97524e002dcbb4101f5934cde992b5721788f7 is pinned literally.  LANDING AMENDMENT
%   (ruling 'port', 2026-07-14, review 5.3.01 code item): the engine's P3 case-split battery is PORTED into the standalone (v4.3),
%   whose VERSION_LOCK corrigenda trail now carries the literal engine SHA; the audit bundle is regenerated (numerically inert) and
%   the concordance text below cites the standalone's case split directly, with the historical dominance line retained as a rider.
%   .08 RT6 explicitly scoped everywhere to the TOTAL spectator-reduction F6 predicate; the conjunction theorem remains predicate-
%   independent.  Residual ``chiral template'' code-metadata wording is not used as a theorem premise; the paper consistently says
%   Weyl-fermion template and treats RT5 as the separate chirality report.
%   .09 The deferred gauge-template coderef moved inside its theorem so the forced section break no longer creates a near-blank page;
%   .10 generic and SM abelian statements, corollary, kinetic-mixing pointer, appendix verification claims, and release-lock prose synced.
% v5.3 (2026-07-15): REVIEW 5.2.01 PASS (MINOR REVISION -- acceptance named as the next step; the reviewer reproduced the
%   waterfall a third time and audited both git heads). All items executed under the rulings 'proceed as recommended':
%   .01 prop:P3 gains the color-dimension case split (dim 3: 3+4+1=8; dim 6: 10; dim 8: 12 -- all > 22/3; reviewer's table
%   verified exactly) and the ENGINE P3 battery is extended from the hard-coded (3,4,1) to the same split (count-neutral);
%   .02 the one-U(1) uniqueness claim MOVED to the physical quotient T_phys = T_rel/ker(rho_M) (the kernel countermodel
%   c=(6Y,0) verified: distinct torus directions induce one physical action) -- H3 clause two, Grades, Steps 4-5, and the
%   Conclusion adjusted; CM removes the inactive kernel, uniqueness is of the EFFECTIVE ray; .03 the B-L edge-case item (b)
%   REWRITTEN -- the 'linear combination' claim was FALSE (Y and B-L linearly independent; all six anomaly conditions incl.
%   mixed vanish -- computed); the licensed exclusion is the EC/CM model-selection premise, aligned with rem:nuR_extended;
%   .04 lem:min_abelian restated conditionally (sufficiency = an anomaly-compatible character of the specified torus action,
%   injective on deficient classes; the R-cardinality proof line and the binary-bit clause removed; downstream (ii)->(iii)
%   reference fixed); .05a C5 recounted (FIVE perturbative conditions: F4's [SU(3)]^3 + F6's four; Witten separate);
%   .05b 'chiral (fermion) template' -> 'Weyl-fermion template' (8 supp + 5 main sites; chirality is RT5's separate report);
%   .05c 'four distinct charges' -> six values with the intended point restated; .05d the twistor dimension sentence fixed
%   (CP^3 finite-dimensional); .05e the 11/3 'massive spin-1' gloss replaced by the sourced paramagnetic/diamagnetic split
%   (+4C_A - C_A/3; Hughes 1980, Nielsen 1981 -- new bibitems); .06 the release VERSION_LOCK split into the numerical-kernel
%   commit (5bc6193, where the scan numbers were locked) + an explicit count-neutral corrigenda trail; .07 the residual
%   7.16pt overfull in the F6 paragraph repaired (header shortened). Prior changelog follows.
% v5.2 (2026-07-14): REVIEW 5.1.01 PASS (second consecutive non-reject; the reviewer independently reproduced the canonical
%   waterfall and credited the version-locked release). All 12 verified items executed; principal rulings: item .11 'do both'
%   (engine metadata re-scope + P3 strengthened in-check), item .04 'conditionalize' (one-U(1) closure conditional on I-typing's
%   action clause; the relative-torus-matter-action lane chartered separately).
%   .01 descriptive-reading corrected: the admissibility predicate leaves FIVE feasible classes; minimality (C9) selects the unique
%   45-DOF SM minimum; .02 F6 retyped honestly: 'electromagnetic completeness' RETIRED as the name (at Y_Q=0 the 42's quark-doublet
%   components carry Q_em = +-1/2 -- computed; colored matter is not electromagnetically neutral there); F6 = the independent
%   phenomenological requirement of nonzero quark-doublet hypercharge, all six sites swept incl. abstract, front ledger, C8, waterfall,
%   thm:Tfield; .03 chirality decoupled from F6 at both sites (the 42 is chiral at its forced Y_Q=0 point; the winner's chirality is
%   verified separately in the release audit layer); .04 the one-U(1) closure CONDITIONALIZED on I-typing clause two -- the
%   relative-torus matter action (central-character matrix c with c(u^c) != c(d^c) and the hypercharge ray in its image) is INPUT,
%   not derived: the type-canonical invariants (n-ality, isospin parity) coincide on u^c/d^c and force Y_u = Y_d, so the naive
%   basis parenthetical in Step 4 was wrong and is replaced by the owned obstruction; Grades line updated (sufficiency +
%   uniqueness-of-direction legs now conditional on H3's action clause); .05 generation universality PROMOTED to a named premise
%   (C3 extended; carried into H3; Step 5 pointer) and the L_mu-L_tau direction owned exactly in the edge-case register with all six
%   anomaly conditions incl. the mixed Y^2 X and Y X^2; .06 the rational-grading lemma retitled and re-scoped (compact-character-lattice
%   premise in the statement; selected-template scope explicit); .07 the Lie-carrier claim scoped to the declared tested list at both
%   sites (the cost ranking removes GROUPS by dimension -- carrier-independent -- and says nothing about untested carriers of
%   surviving groups); .08 the joint N_c/template optimum demoted to its family-scoped form (table retitled/re-captioned; the
%   simultaneous-result sentence rewritten; no enumeration exists at N_c = 5, 7); .09 P3 REPROVED a priori by the reviewer's F2+F5
%   joint budget argument (colored doublet 3 + colorless triplet 4 + parity-restoring >= 1 gives >= 8 > 22/3; verified exactly) --
%   no scan output consumed; appendix echo updated; .10 the computational-notes script description rewritten to the shipped
%   canonical executable (canonical 10/5, RT1-RT6, --emit-audit) with the 5,040-ordering sentence now backed by RT6; the v2
%   predicate attribution moved to the Zenodo archive line; .11 executed bank-side (gauge.py check_T_field metadata re-scoped to
%   the declared-ansatz form + P3 strengthened in-check; see the engine changelog); .12 layout overflows repaired (Thm 4.13
%   derivation chain; bridge ledger). Prior changelog follows.
...
the F6 non-degeneracy requirement (nonzero quark-doublet
hypercharge, $Y_Q \neq 0$ on the complete anomaly system --- an
independent phenomenological assumption), and the Weyl-component objective, the Standard-Model five-multiplet template at 45 Weyl
...
scanned matter content and the typed-inventory input (I-typing,
including the relative-torus matter action of H3, on which the
sufficiency and direction legs are conditional), at least one abelian
...
its corpus homes.  Reproducibility is contractual: the finite waterfall is reproduced
in-tree and by a standalone exact-arithmetic script; the strengthened
P3 case split is proved here and executed in the engine check identified
in Appendix~\ref{app:code}.  The premise list itself --- why
...
\begin{enumerate}[nosep,label=\arabic*.,leftmargin=2em]
  ...
  classification theorem with its full input ledger (C1--C9) --- the
  citable core.
...
Quark-doublet hypercharge non-degeneracy & F6 (canonical: full-system, $Y_Q \neq 0$) & \emph{independent phenomenological assumption}, declared at point of use (\S\ref{sec:seven_filters}); motivated by, not derived from, the admissibility semantics; logically independent of EC-label-injectivity --- computed \\
...
$T_{\mathrm{field}}$ (\S\ref{sec:fermion_scan}) & C1--C9 ledger (Thm.~\ref{thm:conditional_classification}) & closure theorems, $L_{\mathrm{count}}$ \\
...
($T_{\mathrm{field}}$: 6 flavors from the scan, under the C1--C9
...
making the antiscreening origin more transparent: in the
background-field decomposition the gluon's chromomagnetic spin
coupling contributes paramagnetically ($+4\,C_A$, antiscreening)
while the orbital (diamagnetic) part screens ($-\tfrac{1}{3}C_A$),
giving the net $+(11/3)C_A$~\cite{Hughes1980,Nielsen1981}.
...
structure.  The $(11/3)C_A$ decomposes, in the sourced account, into
a paramagnetic spin contribution ($+4\,C_A$, antiscreening) and a
diamagnetic orbital contribution ($-\tfrac{1}{3}C_A$, screening); the
capacity analogue reads the dominant spin term as the cost of
maintaining the self-interacting channel structure and the orbital
term as the ordinary screening overhead.  Paper~4A makes this
...
the spin and orbital contributions (the same $d = 4$ that the APF derives in
...
different from the APF (the twistor correspondence works with
holomorphic data on $\mathbb{CP}^3$ --- itself finite-dimensional,
with the infinite dimensionality residing in the associated function
and cohomology spaces --- while the APF works with finite-dimensional
...
\begin{enumerate}[label=\textbf{BA\arabic*},leftmargin=3em]
  ...
  satisfies the Hawking--King--McCarthy\slash{}Malament distinguishing
  conditions.
...
rows of the orthogonal series.  The cost ranking (under the
operational premise pair of
Proposition~\ref{prop:generator_separation}) removes \emph{groups}
whose dimension exceeds the surviving bound --- a criterion
independent of carrier choice, so it removes every carrier of those
groups at once --- but it says nothing about untested carriers of the
groups it keeps alive.  The classification result is therefore a
minimum within the declared tested carrier list, not a
...
\begin{enumerate}[nosep,label=(\roman*),leftmargin=1.8em]
  ...
  \item $\mathcal C(N_c)$ is the set of Weyl-fermion templates with one
...
in \S6.''  The composite statement is family-scoped: even $N_c$ is
removed by the Witten filter within the family shape, odd $N_c \ge 5$
by family monotonicity of $J(N_c)$, and the full template enumeration
exists at $N_c = 3$ only.  What Propositions~\ref{prop:witten}
and \ref{prop:Nc3} together with Theorem~\ref{thm:Tfield} establish is
therefore: $(N_c = 3, \text{SM template})$ is the unique minimum-cost
admissible pair \emph{within the SM-family-shaped template class}, with
the template claim exhaustive at $N_c = 3$ alone.  A joint optimization
over all template shapes at every rank would require the corresponding
enumeration at $N_c = 5, 7, \ldots$, which is not supplied.
...
\begin{enumerate}[label=\textbf{U\arabic*},nosep]
  ...
  \item The color-rank comparison is family-scoped:
  Table~\ref{tab:joint_Nc_template_optimum} audits SM-family-shaped
  candidates per rank; the full template enumeration exists at
  $N_c = 3$ only (\S\ref{sec:fermion_scan}).
\end{enumerate}
...
\toprule
\textbf{Rank} & \textbf{SM-family-shaped candidate at that rank} & \textbf{Minimum ledger} & \textbf{Reason not selected / selected} \\
\midrule
...
$N_c=3$ & SM template: $Q,u^c,d^c,L,e^c$ & $J=61$ & Family-scoped winner: Witten-even doublet count, anomaly closure, minimum active channels; template exhaustiveness at this rank only. \\
...
\bottomrule
\end{tabular}
\captionof{table}{Per-$N_c$ family-scoped audit.  Each row's candidate is the best \emph{SM-family-shaped} template at that rank; the composite statement is family monotonicity plus the $N_c = 3$ enumeration.  It is not a joint optimization over all template shapes at every rank: no template enumeration is supplied at $N_c = 5, 7, \ldots$}
...
Anomaly algebra alone does not exclude generation-dependent abelian
directions.  On the full three-generation content, with no
right-handed neutrinos, the direction $X = L_\mu - L_\tau$ (charge
$+1$ on the second-generation lepton doublet, $-1$ on its charged
singlet, mirrored with opposite signs for the third generation, zero
elsewhere) satisfies, exactly,
\[
  [\SU(3)]^2 X = [\SU(2)]^2 X = [\mathrm{grav}]^2 X
  = X^3 = Y^2 X = Y X^2 = 0,
\]
and introduces no new fermion component.  What excludes such
directions from the minimal theorem is the generation-universality
convention (C3 of Theorem~\ref{thm:conditional_classification},
carried into H3): all continuous abelian charges act identically on
the three generation multiplicity spaces, because the minimal template
treats generations as repeated copies of one module and pays no
admissibility channel for ``muon family'' versus ``tau family'' as a
primitive gauge label.  Under that named convention the abelian
anomaly space on the types is one-dimensional; without it, it is not.
Any theorem about a product $\U(1)_Y \times \U(1)_X$ must state the
mixed conditions $Y^2 X$ and $Y X^2$ alongside the pure ones, as
above.  A theory that postulates generation-resolving structure as
input lies outside the minimal APF theorem rather than contradicting
it.
...
Within the declared tested carrier list
(\S\ref{sec:R1_classification}; its boundary is stated there --- no
highest-weight exhaustiveness theorem over all faithful carriers is
claimed), no alternative compact nonabelian structure satisfies both
carrier requirements, and no simple-group envelope in that list is
...
\smallskip\noindent\textit{Derivation chain:}
\begin{align*}
&\text{A1} \to \text{Theorem R (R1+R2+R3)} \to
\text{Lie classification} \to
\SU(N_c) \times \SU(2)\\
&\quad (+\text{ one abelian grading, R3})
\to \text{Witten} + \text{minimality} \to N_c = 3 \to
...
representation enlargements unconditionally, P3 excludes colorless
triplets a priori (the joint F2$+$F5 budget argument), P4--P5 certify
dominance against the named extension directions, and the 1{,}680
candidates exhaust the declared space.  The caps themselves are declared scan
...
exceeds the AF budget regardless of the rest of the template.  P3 excludes
colorless triplets a priori (the joint F2$+$F5 budget argument,
Proposition~\ref{prop:P3}), P5 is a dominance certificate against
field types beyond the anomaly-closed core, and P4 is the
two-colored-doublet class-dominance result (class minimum 54,
Proposition~\ref{prop:P4}).  Within the declared
...
define a predicate
$\mathrm{Adm}(\mathcal{T})$ on Weyl-fermion templates.  The scan
is an \emph{algorithmic verification} that the set
$\{\mathcal{T} : \mathrm{Adm}(\mathcal{T})\}$ consists of exactly
five CPT-inequivalent feasible classes (at 45, 48, 66, 69, and 72
Weyl components), and that the separate minimality objective (C9)
selects a unique member from among them --- the 45-Weyl-fermion SM
...
therefore descriptive: ``the admissibility predicate leaves five
feasible classes, of which the SM template is the unique
Weyl-count minimum,'' not ``the framework selects one template
from among 1{,}680.''
...
unconditionally, P3 excludes colorless triplets a priori (F2$+$F5),
P4--P5 certify dominance against the named extension directions, and
the caps themselves are declared scan inputs whose completeness is an
...
A \emph{Weyl-fermion template} is a finite multiset
...
(imported at its banked ground, Paper~2 \S9).
...
\label{prop:P1}
Every irreducible $\SU(3)$ representation of dimension at least $10$
has Dynkin index at least $15/2$.  Consequently one such
$\SU(2)$-singlet Weyl entry, replicated over three generations,
already contributes at least
\[
  \frac{2}{3}\,\frac{15}{2}\,(1)(3)=15>11
\]
to the $\SU(3)$ one-loop matter term, so it is incompatible with F1.
The only irreducible color representations below dimension $10$ are
$\mathbf1,\mathbf3,\bar{\mathbf3},\mathbf6,\bar{\mathbf6},\mathbf8$.
\end{proposition}

\begin{proof}
Write an $\SU(3)$ irrep by Dynkin labels $(p,q)$, $p,q\in\mathbb Z_{\ge0}$.
The standard formulas are~\cite{FultonHarris1991}
\[
...
\[
 ...
 \qquad
 T(p,q)=\frac{\dim(p,q)\,C_2(p,q)}{8}.
\]
For $p+q\le2$ these give exactly
$(0,0)$, $(1,0)$, $(0,1)$, $(2,0)$, $(0,2)$, and $(1,1)$, of dimensions
$1,3,3,6,6,8$.  If $p+q=3$, the boundary irreps
$(3,0)$ and $(0,3)$ have dimension $10$, $C_2=6$, and
$T=15/2$, while $(2,1)$ and $(1,2)$ have dimension $15$ and
$T=10$.  If $p+q\ge4$, then $\dim(p,q)\ge15$ and
$C_2(p,q)\ge8$, hence $T(p,q)\ge15$.  Thus the minimum index among
all irreps of dimension at least $10$ is $15/2$, attained precisely by
$\mathbf{10}$ and $\overline{\mathbf{10}}$.  Under the all-left-handed
Weyl convention the coefficient is $2/3$ per entry (the $4/3$ form is
for a Dirac pair), giving the stated AF violation.  Adding further
fields only decreases the AF margin.
\end{proof}
...
\begin{proposition}[P3: Colorless $\SU(2)$ triplets are excluded by
asymptotic freedom and Witten parity]
\label{prop:P3}
No admissible template contains a colorless $\SU(2)$ triplet.  The
proof is a joint F2$+$F5 budget argument, a priori --- it consumes no
scan output.  Any template passing F3 carries at least one colored
doublet, whose color representation ranges over the declared list
$\{\mathbf{3}, \bar{\mathbf{3}}, \mathbf{6}, \bar{\mathbf{6}},
\mathbf{8}\}$.  Suppose exactly one.  In the $\SU(2)$
asymptotic-freedom ledger, each entry contributes
$(2/3)\,T(r^{(2)})\dim(r^{(3)})\,N_{\mathrm{gen}}$, so the colored
doublet contributes its color dimension and a colorless triplet
($T(\mathbf{3}) = 2$) contributes $4$.  The case split over the color
dimension:
\begin{center}\small
\begin{tabular}{@{}rrrp{6.4cm}@{}}
\toprule
$\dim r^{(3)}$ & doublet & triplet & result \\
\midrule
$3$ & $3$ & $4$ & $7 < 22/3$, but the doublet count ($3$) is Witten-odd; the cheapest parity repair (a colorless doublet) adds $1$: $3 + 4 + 1 = 8 > 22/3$ \\
$6$ & $6$ & $4$ & $6 + 4 = 10 > 22/3$ already (parity even) \\
$8$ & $8$ & $4$ & $8 + 4 = 12 > 22/3$ already (parity even) \\
\bottomrule
\end{tabular}
\end{center}
In every case F2 and F5 cannot hold jointly.  Hence no
one-colored-doublet template containing a colorless triplet is
admissible.  Templates with two or more colored doublets are dominated
by Proposition~\ref{prop:P4} (class minimum 54); templates with no
colored doublet fail F3.
\end{proposition}
...
\label{prop:P5}
Dominance form (cf.\ P4).  The a-priori per-type floor ($\ge 3$ DOF:
...
sub-45-DOF template exists outside them --- is carried by P3's budget exclusion and P5-style
...
\times 2 \times 3 = 1{,}680$.  The canonical standalone verification script carries exact rational
arithmetic and six built-in red-team challenges (RT1--RT6); its v2
predecessor is archived at \url{https://doi.org/10.5281/zenodo.19154197}; an
...
F6 additionally carrying the independent $Y_Q \neq 0$ non-degeneracy
...
\begin{enumerate}[label=\textbf{F\arabic*},nosep,leftmargin=3em]
  ...
  colored-doublet partner can occur.  Chirality itself is settled by
  no filter: it is a charge-dependent property (absence of
  gauge-invariant conjugate pairings), the excluded 42-DOF template is
  chiral at its forced $Y_Q = 0$ point, and the winner's chirality is
  verified separately, at its computed witness, in the release's audit
  layer.  A true chirality predicate ---
  ...
  \item \textbf{Non-degenerate full anomaly system.}\quad
  ...
  excludes it by F6's \textbf{non-degeneracy requirement}: the
  quark-doublet hypercharge must be nonzero, $Y_Q \neq 0$, so the
  anomaly system is required to be \emph{non-degenerate}, not merely
  solvable.  This is hereby declared an \emph{independent
  phenomenological assumption} of the scan at exactly this point of
  use.  An earlier name for it, ``electromagnetic completeness,''
  overstated what the predicate does: under
  $Q_{\mathrm{em}} = T_3 + Y$ the excluded template's quark-doublet
  components carry electric charges $\pm 1/2$ and its colored
  singlets $\pm t$ even at $Y_Q = 0$ --- colored matter is not
  electromagnetically neutral there, and the forced solution is
  label-injective (computed).  The predicate is exactly what it
  computes: nonzero quark-doublet hypercharge on the complete anomaly
  system.  The admissibility semantics motivates it (a
  hypercharge-blind quark doublet leaves the abelian grading doing no
  separating work on the colored-doublet type), but that motivation
  does not derive it: it is not a consequence of anomaly cancellation,
  and it does the work of removing the 42-DOF near-miss.  A reader who
  rejects it gets the stripped generation as a second survivor below
  the SM.  The stripped template is recorded
  ...
  (the predicate of the archived v2 scan, Zenodo
  \texttt{10.5281/zenodo.19154197}; the shipped canonical script
  reports it as a robustness statistic): strip anomaly-free spectators
...
positions, under the canonical full-system F6 predicate --- exactly
what the canonical standalone script computes (1{,}680 distinct
multisets via \texttt{combinations\_with\_replacement}; canonical
F6 primary).  The in-tree \texttt{check\_T\_field} in
\texttt{gauge.py} uses an ordered-tuple enumeration
(\texttt{itertools.product}) and therefore tests each distinct
template multiple times, producing a raw iteration count of
$4{,}680$ rather than $1{,}680$; its F6 convention is the
uniform-dimension robustness variant ($4 \to 2$ after quotienting
the ordering redundancy).  The spectator-reduction variant
($8 \to 4$) is the archived v2 script's predicate, reported by the
canonical script as a robustness statistic (the predicate statement
in \S\ref{sec:seven_filters} states all three conventions and
designates the canonical one).  The minimality winner is identical
under either enumeration and every stated F6 convention: the Standard
Model at 45 DOF.  A reviewer running the code
...
checks.  \emph{Spectator reduction}, reported by the shipped
standalone scan (\texttt{fermion\_scan\_standalone.py}) as a
robustness statistic (it is the archived v2 implementation's
predicate), strips the
...
set, however, is order-invariant.  The canonical standalone script
confirms this computationally (red-team check RT6): running all
$7! = 5{,}040$ filter orderings on the 1{,}680-template set under the
total spectator-reduction predicate yields the same 4 post-CPT
survivors (and the same unique DOF-45 winner) in every ordering.  The conjunction argument is
...
enumeration caps (\S\ref{sec:exclusion_proofs}): P3 excludes
colorless triplets a priori (F2$+$F5), P5 is a dominance result
certifying that extra field types cannot lower the minimum, and P4 is the
...
(the $Y_Q \neq 0$ non-degeneracy requirement, an independent
assumption, \S\ref{sec:seven_filters}); the F6-free argmin is the 42-DOF stripped
...
Within the declared representation list and multiplicity caps of
\S\ref{sec:exclusion_proofs}, and under the seven filters and imported
inputs stated in Theorem~\ref{thm:conditional_classification} --- in
particular the fixed gauge template
$\SU(3)\times\SU(2)\times\U(1)$, three generation copies, and the
independent F6 non-degeneracy requirement $Y_Q\neq0$ in its canonical
full-system form --- the unique minimum among the enumerated
Weyl-fermion templates is
\[
  \{Q(\mathbf{3},\mathbf{2}),\;
...
\]
with $\mathrm{DOF}=(6+3+3+2+1)\times3=45$ Weyl components.
No completeness claim is made outside the declared finite ansatz.
\end{theorem}

\noindent No other template among the 1{,}680 candidates has
$\mathrm{DOF} \le 45$ and passes all seven filters.  P1--P2 exclude
the enumerated representation enlargements unconditionally; P3
excludes colorless triplets a priori (F2$+$F5); P4--P5 certify
dominance for the enumerated extension directions; the caps
...
\begin{enumerate}[label=(C\arabic*),nosep]
  ...
  \item \textbf{Generation count and generation universality.}\quad
  $N_{\mathrm{gen}} = 3$, imported
  (\S\ref{sec:three_generation_scope}) and consumed by the F1--F2
  filters; and the convention, load-bearing throughout, that
  generations enter as indistinguishable multiplicity copies ---
  \emph{all continuous abelian charges act identically on the three
  generation multiplicity spaces}.  The scan's per-generation data
  structure and the abelian-sector theorems both consume this
  convention; relaxing it admits generation-dependent anomaly-free
  directions (the edge-case register of \S\ref{sec:one_u1_closure}
  owns the exact example).
  ...
  (\S\ref{sec:beta_schematic}).
  \item \textbf{The anomaly system.}\quad The five perturbative
  anomaly conditions --- the pure $[\SU(3)]^3$ condition (F4) and the
  four hypercharge-system equations (F6) --- together with the Witten
  global-anomaly parity condition (F5), consumed as filter conditions; their solutions are
  ...
  the winner's chirality is verified separately in the release's
  audit layer, not by any filter).
  \item \textbf{CPT identification.}\quad F7's convention that
  conjugate templates describe the same physics.
  \item \textbf{F6 non-degeneracy ($Y_Q \neq 0$).}\quad An
  independent phenomenological assumption --- nonzero quark-doublet
  hypercharge on the complete anomaly system --- declared at its point
  of use (\S\ref{sec:seven_filters}), where its honest typing is
  stated (the retired name ``electromagnetic completeness'' overstated
  it); without it the argmin moves to the 42-DOF stripped generation.
  F6 and EC-label-injectivity are logically independent --- computed,
  in both directions\coderef{check\_L\_F6\_not\_from\_EC}{ec\_inventory\_reading.py}.
  ...
  degrees of freedom.
\end{enumerate}
Under (C1)--(C9), the Standard-Model five-multiplet template at 45
...
premises: (C8) is an independent assumption; (C1)'s caps, (C7), and
(C9) are declared conventions of the enumeration; (C4) and (C5) are
imported standard results; (C2) and (C3) are imported APF results
consumed as inputs.
\end{theorem}

\noindent\textit{Why the equal-unit convention is not a C-premise.}\quad
The fixed-gauge fermion argmin compares only Weyl-component counts, so
it does not use an equivalence between Weyl components, Higgs
components, and gauge generators.  The equal-unit channel convention
first becomes load-bearing in $L_{\mathrm{count}}$
(\S\ref{sec:capacity_count}), where the separately fixed terms
$45$, $4$, and $12$ are combined into the 61-channel ledger.
...
given the SM Weyl-fermion template, anomaly cancellation fixes hypercharge up to
...
anomaly conditions applied to a Weyl-fermion template.  APF does not
...
\[
  ...
  Y_e = -1.
\]
These are the unique hypercharges (up to normalization and
$u\leftrightarrow d$) in the right-handed-partner numerical
convention used for $Y_u,Y_d,Y_e$.  The actual left-handed conjugate
fields carry $Y(u^c)=-2/3$, $Y(d^c)=+1/3$, and $Y(e^c)=+1$.
\end{theorem}
...
template-independent at the level of effective rank: for a fixed
non-abelian multiplet set, a specified relative-torus matter action,
and EC/CM, it identifies when a further grading is necessary and when
one effective character suffices.  Theorem~\ref{thm:unique_U1_consolidated} is its SM
specialization.
...
\smallskip\noindent The abelian result is split into two levels.
\textbf{Theorem~A} (Proposition~\ref{prop:unique_U1}) is
structural and template-independent only at the level actually fixed
by the data: the separation deficiency detects whether the
non-abelian labels are insufficient, while a \emph{specified} matter
action of the relative torus determines whether one effective
character can repair every deficient class.  Under EC and CM this
fixes the minimum effective abelian \emph{rank}; it does not, in
general, choose a unique charge ray.  \textbf{Theorem~B}
(Theorem~\ref{thm:unique_U1_consolidated}) is the SM specialization:
it supplies the five-type character action and uses the anomaly
system to select the hypercharge ray up to normalization and the
$u\leftrightarrow d$ relabeling.

\begin{lemma}[Abelian factors arise only from $T_{\mathrm{rel}}$]
\label{lem:abelian_source}
Let
$\mathcal A\cong\bigoplus_{\alpha=1}^{m}M_{n_\alpha}(\mathbb C)$ and
\[
 T_{\mathrm{rel}}:=Z(\mathrm U(\mathcal A))/\mathrm U(1)_{\mathrm{diag}}
 \cong \mathrm U(1)^{m-1}.
\]
Under the gauge-identification hypotheses of \TSII{}, every
independent continuous abelian gauge factor has a representative
one-dimensional subgroup of $T_{\mathrm{rel}}$; no independent
abelian normal factor arises from another source.
\end{lemma}

\begin{proof}
The exact sequence (Proposition~2.19 of \TSII{}) is
\[
  1\longrightarrow Z(\mathrm U(\mathcal A))
  \longrightarrow \mathrm U(\mathcal A)
  \xrightarrow{\pi}\mathrm{Inn}(\mathcal A)\longrightarrow1.
\]
Here $\mathrm{Inn}(\mathcal A)=\prod_\alpha\mathrm{PU}(n_\alpha)$
is semisimple (trivial factors at $n_\alpha=1$ omitted).  It contains
maximal tori, but no connected abelian normal direct factor with an
independent coupling.  A continuous abelian factor commuting with the
full lifted action therefore lies in $\ker\pi=Z(\mathrm U(\mathcal A))$.
After quotienting the physically common diagonal phase, its
representative lies in $T_{\mathrm{rel}}$.  Discrete remnants do not
supply continuous generators, and an external abelian group not
represented through the admissibility algebra would violate the
stated gauge-identification hypothesis.
\end{proof}

\begin{definition}[Separation deficiency]\label{def:sep_deficiency}
For physically distinct matter types
$\mathcal M=\{r_1,\ldots,r_N\}$ carrying irreducible
$G_{\mathrm{na}}$ representations, define
\[
  \delta(G_{\mathrm{na}},\mathcal M)
  :=\bigl|\{(r_i,r_j):i<j,
  \ [r_i]_{G_{\mathrm{na}}}=[r_j]_{G_{\mathrm{na}}}\}\bigr|.
\]
Thus $\delta=0$ precisely when the non-abelian label map is injective.
For $\delta>0$, at least one class of physically distinct types remains
unresolved by the non-abelian labels.
\end{definition}

\begin{lemma}[Minimum effective abelian rank for a specified torus action]
\label{lem:min_abelian}
Let
\[
 \rho_{\mathcal M}:T_{\mathrm{rel}}\longrightarrow U(H_{\mathcal M})
\]
be a continuous matter representation, diagonal on the type sectors.
Choose integral coordinates on $T_{\mathrm{rel}}\cong U(1)^r$ and let
$C\in\mathbb Z^{N\times r}$ be its character matrix, with row
$c(r_i)$.  A cocharacter $q\in\mathbb Z^r$ induces the charge vector
\[
  y(q):=Cq\in\mathbb Z^{\mathcal M}.
\]
Define the effective torus
\[
  T_{\mathrm{phys}}:=T_{\mathrm{rel}}/\ker\rho_{\mathcal M};
\]
two cocharacters represent the same physical action exactly when
their difference lies in the matter-action kernel.  Let
$\mathcal P=\{P_1,\ldots,P_s\}$ be the partition of $\mathcal M$ by
non-abelian labels.  Then:
\begin{enumerate}[label=(\roman*),nosep]
  \item if $\delta=0$, no effective abelian factor is required by EC;
  \item if $\delta>0$, at least one effective abelian factor is
  required by EC;
  \item if $\delta>0$ and there exists a cocharacter class
  $[q]\in X_*(T_{\mathrm{phys}})$ for which $y(q)$ satisfies the
  applicable anomaly/consistency equations and is injective on every
  deficient class $P_k$, then one effective $U(1)$ suffices.
\end{enumerate}
The sufficiency statement is not a consequence of $\delta$ alone: it
uses the specified character action, its compact character lattice,
the relevant consistency equations, and any declared universality
conditions on multiplicity spaces.
\end{lemma}

\begin{proof}
If $\delta>0$, two distinct types have the same non-abelian label.  EC
requires another label source, and Lemma~\ref{lem:abelian_source}
restricts every continuous candidate to the relative torus, proving
(ii).  For (iii), the hypothesized $q$ assigns different scalar
charges within every deficient class, so
$r_i\mapsto([r_i]_{G_{\mathrm{na}}},y_i(q))$ is injective.  One
effective factor therefore suffices.  Clause (i) is the zero-deficiency
special case.
\end{proof}

\begin{proposition}[Minimum effective abelian rank --- structural
(Theorem~A)]\label{prop:unique_U1}
Let $\mathcal A$, $G_{\mathrm{na}}$, $T_{\mathrm{rel}}$,
$\mathcal M$, $\rho_{\mathcal M}$, $C$, and $T_{\mathrm{phys}}$ be as
above, and assume EC and CM at the grades stated in \TSII{}.
Then:
\begin{enumerate}[label=(\roman*),nosep]
  \item if $\delta=0$, the minimum effective abelian rank is zero;
  \item if $\delta>0$ and the separating-character hypothesis of
  Lemma~\ref{lem:min_abelian}(iii) holds, the minimum effective
  abelian rank is one;
  \item once one character makes the augmented label map injective,
  every additional effective abelian generator is redundant under the
  EC/CM vocabulary and is removed by CM; kernel directions are already
  absent from $T_{\mathrm{phys}}$;
  \item no uniqueness of the one-dimensional charge ray is implied
  without an additional hypothesis selecting a unique projective
  solution of the consistency equations.
\end{enumerate}
\end{proposition}

\begin{proof}
The rank-zero and necessity statements follow from
Definition~\ref{def:sep_deficiency} and
Lemma~\ref{lem:min_abelian}.  Under the separating-character
hypothesis, one effective $U(1)$ makes every label class a singleton.
A second effective generator cannot refine a discrete partition, yet
it carries positive channel cost by Lemma~2.22 of \TSII{}; CM therefore
removes it.  This establishes the minimum rank.  Nothing in this
argument compares two distinct separating cocharacters of equal rank,
so it cannot establish a unique direction.
\end{proof}

\noindent\emph{Scope of Theorem~A.}\quad
The rank statement is relative to the EC/CM vocabulary, in which an
abelian generator earns admissibility value by refining the counted
matter-type labels.  A current distinguished only by couplings,
symmetry-breaking history, or other dynamical data not represented in
those labels lies outside this theorem's premise set rather than
constituting a second minimum-rank solution.

\begin{remark}[Why Theorem~A does not select a generic ray]
Take three types with character rows
\[
  (0,0),\qquad(1,0),\qquad(0,1).
\]
Both $q=(1,2)$ and $q=(2,1)$ give three distinct scalar charges and
have the same one-generator cost.  EC and CM select rank one over rank
two, but do not choose between these two rays.  Directional uniqueness
in this supplement is therefore an SM-specialized anomaly result
(Theorem~\ref{thm:unique_U1_consolidated}), not a consequence of
separation deficiency.
\end{remark}

\begin{lemma}[Diagonal grading and rational ratios on the selected template]
\label{lem:A3_derived}
Assume the compact-character-lattice premise, the selected
five-multiplet template, and a specified integral character matrix
$C$ for the relative-torus matter action.  Then every cocharacter acts
by a scalar on each irreducible type sector, and the selected
Standard-Model anomaly system fixes rational charge ratios.
\end{lemma}

\begin{proof}
Central torus elements commute with the non-abelian action, so Schur's
lemma makes their action scalar on each irreducible type sector; in
integral torus coordinates the resulting charges are $Cq$.  On the
selected template, the three linear anomaly equations express
$Y_L,Y_d,Y_e$ in terms of $Y_Q,Y_u$, and the cubic equation reduces to
$z^2-2z-(N_c^2-1)=0$ with $z=Y_u/Y_Q$.  Its roots
$z=1\pm N_c$ are integers.  Hence all ratios are rational.  The
absolute unit remains a compact-character-lattice normalization,
not an anomaly consequence.
\end{proof}

\smallskip\noindent\fbox{\parbox{0.95\linewidth}{\textbf{Sign convention used by the torus action and anomaly equations.}\quad
The matter representation $\rho_{\mathcal M}$ acts on the actual
left-handed sectors $(Q,u^c,d^c,L,e^c)$.  Their hypercharge vector is
\[
  y_{\mathrm{hyp}}^{\mathrm{LH}}
  =\left(\tfrac16,-\tfrac23,\tfrac13,-\tfrac12,1\right)
  \propto (1,-4,2,-3,6).
\]
For conventional tables, this supplement also writes
$Y_u:=Y(u_R)=+2/3$, $Y_d:=Y(d_R)=-1/3$, and
$Y_e:=Y(e_R)=-1$; these are the right-handed-partner numerical values,
so the signs flip relative to $u^c,d^c,e^c$.  Representation
conjugation and the $U(1)$ sign always change together.}}
\smallskip

\begin{corollary}[Application to the derived Standard Model template]
\label{cor:unique_U1_SM}
For
$\mathcal M_{\mathrm{SM}}=\{Q,u^c,d^c,L,e^c\}$, the non-abelian
labels under $\SU(3)\times\SU(2)$ conflate exactly the pair
$u^c,d^c$, so $\delta=1$.  Given the H3 relative-torus action of
Theorem~\ref{thm:unique_U1_consolidated}, whose rational character map
realizes hypercharge, Proposition~\ref{prop:unique_U1} implies that the
minimum effective abelian rank is one.  The uniqueness of the
hypercharge ray itself is the additional SM anomaly result of
Theorem~\ref{thm:unique_U1_consolidated}, not a generic consequence of
$\delta$.
\end{corollary}
...
\begin{enumerate}[label=(H\arabic*),nosep]
  ...
  of the entry-typed inventory I-typing, which carries two clauses.
  \emph{Clause one (type distinctness):} the five listed multiplets
  ...
  \emph{Clause two (the relative-torus matter action):} the action of
  $T_{\mathrm{rel}}\cong\U(1)^2$ on the five left-handed type
  sectors is itself input data.  Write
  $c(r)\in\mathbb Z^2$ for the integral character row of type $r$ and
  assemble the rows into
  \[
    C\in\mathbb Z^{5\times2},\qquad
    C_{r\bullet}=c(r).
  \]
  A cocharacter $q\in\mathbb Z^2$ induces the all-left-handed charge
  vector
  \[
    y(q)=Cq\in\mathbb Z^{\mathcal M};
  \]
  after rational extension, write
  $C_{\mathbb Q}:\mathbb Q^2\to\mathbb Q^{\mathcal M}$.  The required
  input is
  \[
    c(u^c)\ne c(d^c),\qquad
    y_{\mathrm{hyp}}^{\mathrm{LH}}
    \in\operatorname{im}C_{\mathbb Q},
    \quad
    y_{\mathrm{hyp}}^{\mathrm{LH}}
    \propto(1,-4,2,-3,6),
  \]
  in the ordered basis $(Q,u^c,d^c,L,e^c)$.  This clause is
  load-bearing and cannot be replaced by the type-canonical
  non-abelian invariants: $SU(3)$ $n$-ality and $SU(2)$ isospin parity
  coincide on $u^c$ and $d^c$, so any action factoring only through
  those invariants forces equal charges on that pair.  Generation
  universality (C3 of Theorem~\ref{thm:conditional_classification}) is
  part of the same clause: $C$ is a function of type and acts
  identically on the three generation copies.

  Let $\rho_{\mathcal M}$ be the matter representation defined by
  $C$ and
  \[
    T_{\mathrm{phys}}:=T_{\mathrm{rel}}/\ker\rho_{\mathcal M}.
  \]
  Cocharacters differing by the kernel induce the same charge vector,
  so the closure's uniqueness statement concerns an effective ray in
  $T_{\mathrm{phys}}$, not a distinguished subgroup of the
  unquotiented torus.  CM removes any inactive kernel direction.
...
determined \emph{effective charge ray in the physical quotient}
$T_{\mathrm{phys}} = T_{\mathrm{rel}} / \ker\rho_{\mathcal{M}}$
(H3): directions of $T_{\mathrm{rel}}$ differing by the matter-action
kernel induce the same physical action, and the inactive kernel is
removed by CM.  This factor is hypercharge $\U(1)_Y$.  In the
right-handed-partner numerical convention defined above,
\[
  (Y_Q,Y_u,Y_d,Y_L,Y_e)
  =\left(\tfrac16,\tfrac23,-\tfrac13,-\tfrac12,-1\right),
\]
up to normalization and the $u\leftrightarrow d$ relabeling.  On the
actual left-handed sectors $(Q,u^c,d^c,L,e^c)$ on which
$\rho_{\mathcal M}$ acts, the same ray is
\[
  y_{\mathrm{hyp}}^{\mathrm{LH}}
  =\left(\tfrac16,-\tfrac23,\tfrac13,-\tfrac12,1\right).
\]
The absolute unit uses the compact-$\U(1)$ character-lattice convention
of the front-matter ledger.
...
the sufficiency and uniqueness-of-direction legs are conditional on
I-typing's action clause (H3): the anomaly system's unique ray is a
fact of the abstract five-charge space, computed unconditionally, and
that $y_{\mathrm{hyp}}^{\mathrm{LH}}$ lies in
$\operatorname{im}C_{\mathbb Q}$ is supplied by H3, not derived ---
and uniqueness is of the effective
ray in the physical quotient $T_{\mathrm{phys}}$ (H3), not of a
subgroup of the unquotiented torus; the CM leg (no second factor,
and the removal of the inactive kernel direction) is [P]
...
$n \ge 2$).  A semisimple group has no nontrivial connected abelian quotient or
normal direct factor.  Every connected abelian subgroup of
$\mathrm{PU}(n)$ is contained in a maximal torus, which is part of the
non-abelian structure rather than an independent normal factor.
...
degeneracies.}\quad
A one-dimensional subgroup $\U(1) \subset T_{\mathrm{rel}}$
represented by a cocharacter $q$ assigns the all-left-handed vector
$y(q)=Cq$, constant on each type sector by Schur's lemma.  On the
selected template the anomaly system fixes rational charge ratios
(\S\ref{sec:hypercharge}).  The augmented label map
$r_i\mapsto([r_i]_{\SU(3)\times\SU(2)},y_i(q))$ is injective iff
$y(u^c)\ne y(d^c)$, equivalently $Y_u\ne Y_d$ in the declared
right-handed-partner convention (this is the only unresolved pair
from Step~2; the other three types already have distinct
non-abelian labels).  The anomaly system
...
\textit{Step 4 (Uniqueness of direction): the anomaly system admits a
unique ray, realized as an effective ray in $T_{\mathrm{phys}}$ by
H3's action clause.}\quad
...
positive rescaling).  The all-left-handed charge of multiplet $r_i$ is
$y_i(q)=q_1c_1(r_i)+q_2c_2(r_i)$, equivalently $y(q)=Cq$; the
symbols $Y_u,Y_d,Y_e$ used in the anomaly equations are obtained by
the fixed right-handed-partner sign convention stated above.
The type-canonical invariants cannot serve as $(c_1, c_2)$: $\SU(3)$
$n$-ality and $\SU(2)$ isospin parity coincide on $u^c$ and $d^c$,
forcing $Y_u = Y_d$ on every direction, which contradicts Step~3.
That the action separates $u^c$ from $d^c$ and realizes the
hypercharge ray is exactly what clause two supplies.
...
$z = 4$ and $z = -2$.  These two roots give charge rays exchanged by the
$u\leftrightarrow d$ relabeling symmetry of the fermion template.
Therefore the anomaly system admits one projective physical ray in the
abstract five-charge space, up to normalization and that discrete
relabeling --- an unconditional computed fact.  Given H3's action clause, a realizing
cocharacter exists because
$y_{\mathrm{hyp}}^{\mathrm{LH}}\in\operatorname{im}C_{\mathbb Q}$,
and what is unique is its class in the physical quotient: distinct directions of $T_{\mathrm{rel}}$ differing by
$\ker\rho_{\mathcal{M}}$ induce identical matter charges, so
uniqueness of a \emph{subgroup of the unquotiented torus} does not
follow from the stated properties and is not claimed --- the claim is
the unique effective ray in
$T_{\mathrm{phys}} = T_{\mathrm{rel}} / \ker\rho_{\mathcal{M}}$.  (``Admits'' rather than ``selects'': no
...
all five multiplet types (the all-left-handed vector
...
a second left-handed charge $y'_i$ to each multiplet.  The augmented
map $r_i\mapsto([r_i],y_i,y'_i)$ induces the same discrete
...
capacity (A1), this is inadmissible.  The same argument removes any
inactive kernel direction of the matter action: a generator in
$\ker\rho_{\mathcal{M}}$ assigns zero to every multiplet, carries
one channel at cost $\varepsilon^* > 0$, and separates nothing ---
which is why the uniqueness claim lives in $T_{\mathrm{phys}}$.
(Generation universality is
load-bearing here: the label map and the anomaly system are stated on
the five \emph{types}, i.e., all continuous abelian charges act
identically on the three generation copies --- C3 of
Theorem~\ref{thm:conditional_classification}, carried into H3.
Relaxing it admits generation-dependent anomaly-free directions; the
edge-case register below owns the exact example.)
...
the anomaly system admits a unique ray, realized in
$T_{\mathrm{rel}}$ by H3's action clause --- uniquely as an effective
ray in $T_{\mathrm{phys}}$; and no second factor is
...
\footnotesize
Starting from $\mathcal A=M_3(\mathbb C)\oplus M_2(\mathbb C)\oplus\mathbb C$, the nonabelian inner automorphism group contributes $\mathrm{PU}(3)\times\mathrm{PU}(2)$ and no central $\U(1)$.  The unitary-center quotient leaves a two-dimensional relative-phase torus.  The derived matter template contains two multiplets with the same nonabelian label, $u^c$ and $d^c$, so admissibility completeness requires at least one relative phase.  The standard anomaly equations on the scanned template single out one rational projective ray in charge space, hypercharge; that the relative torus acts on the matter types by characters realizing that ray is part of the typed-inventory input (H3), not a consequence of the algebra.  Once this ray is included, all five multiplets are separated; any second effective factor either lies outside the anomaly-compatible solution space or separates no additional minimal-template distinction under EC/CM.  MD assigns that extra generator positive cost, so A2 removes it.  This is the whole no-redundancy proof, stated without APF shorthand.
\end{tcolorbox}
...
\begin{itemize}[nosep]
  ...
  related by the relabeling symmetry).  That the solution ray is
  realized by the relative torus rides H3's action clause, an input.
  ...
  $\varepsilon^* > 0$).
\end{itemize}
The conditional inputs are the matter content
$\mathcal{M} = \mathcal{M}_{\mathrm{SM}}$, derived in
$T_{\mathrm{field}}$ (\S\ref{sec:fermion_scan}), and I-typing's
action clause --- the relative-torus central characters (H3).  Given
those inputs, the conclusion is a theorem, not a selection.
...
\begin{enumerate}[label=(\alph*),nosep]
  ...
  $\mathcal{M}' = \mathcal{M}_{\mathrm{SM}} \cup \{\nu_R\}$ were the
  derived template.  In $\mathcal{M}'$, $\U(1)_{B-L}$ is an
  \emph{independent} anomaly-free direction: $Y$ and $B{-}L$ are
  linearly independent charge vectors on the six types, both satisfy
  the four pure anomaly conditions, and the mixed conditions
  $Y^2(B{-}L)$ and $Y(B{-}L)^2$ vanish as well (computed, exact); nor
  can an $\SU(2)_L$ or color Cartan admixture bridge them, since a
  charge constant on each irreducible multiplet forces those
  coefficients to zero.  What excludes a second \emph{massless
  gauged} factor is therefore not algebraic dependence but the
  explicit EC/CM convention: the non-abelian labels conflate
  $u^c / d^c$ (both $(\bar{3},1)$) and $e^c / \nu_R$ (both
  $(1,1)$); one scalar coordinate resolves both degeneracy pairs
  (hypercharge assigns six distinct values on $\mathcal{M}'$); and a
  second current supplies no further label-partition gain while
  carrying one additional admissibility channel.  That is a
  model-selection premise of the admissibility semantics, stated as
  such --- consistent with
  Remark~\ref{rem:nuR_extended_v35}, which records that the extended
  abelian solution space is genuinely larger.
...
and even if $\nu_R$ were included, $B{-}L$ --- though an independent
anomaly-free direction there --- is excluded by the EC/CM
model-selection premise, not by algebraic dependence.
...
Theorem~\ref{thm:unique_U1_consolidated} leaves no second low-energy factor to mix with.  The
...
Theorem~\ref{thm:hypercharge} --- stated on the five multiplet types
under the generation-universality convention (C3/H3) --- with the
scanned representation content held fixed and no sterile singlet
added, has a one-dimensional solution space; after minimal integral
normalization this solution is precisely the hypercharge ray.  Thus an
independent $q$ is not anomaly-compatible on that template.
...
in charge space, realized in $T_{\mathrm{rel}}$ under H3's action clause (I-typing).  Minimal charge quantization fixes the primitive
...
Every theorem with executable finite content is mapped to a named
check function.  The checks certify the programmed arithmetic,
finite predicates, witnesses, and dependency metadata at the stated
scope; they do not replace the mathematical proofs or establish the
physical necessity of named premises.  The table below gives the
mapping.  Rows whose section column points into \TSII\ are consumed
inputs rather than results re-proved by execution here.
...
$L_{\mathrm{gauge,unique}}$ & \S\ref{sec:gauge_class_supp} & \codeid{check\_L\_gauge\_template\_uniqueness} & \texttt{gauge.py} \\
...
The numerical scan kernel is locked at engine commit
\codeid{5bc619370755f541cf122a20e0f51f9f5b5f97f9}; the current
proof-scope corrigendum carrying the complete P3 color-dimension case
split is
\codeid{ff97524e002dcbb4101f5934cde992b5721788f7}.  Later
proof-scope corrections are count-neutral relative to the certified
waterfall.
...
files.  The strengthened exclusion arguments are split by scope:
\texttt{check\_T\_field} at commit
\codeid{ff97524e002dcbb4101f5934cde992b5721788f7} executes the full
P3 color-dimension case split as well as the P4 witness battery, and
the standalone v4.3 mirrors the same P3 case-split battery (dimensions
3, 6, 8, with the dim-3 Witten parity repair; the historical dominance
comparison is retained as a rider only); the text supplies the
representation-theoretic completion of P1.  Hypercharge and
anomaly arithmetic use exact \texttt{fractions.Fraction} values.  Running
...
is a self-contained Python script (zero dependencies beyond the
standard library) that reproduces the complete finite scan from
scratch.  Its objects are Weyl-fermion templates; the legacy word
``chiral'' in one code-header sentence is metadata only, while RT5 is
the separate chirality report.  It is the canonical numerical
executable of the release: every count in the waterfall table is
generated by it, and
\texttt{python fermion\_scan\_standalone.py --emit-audit} writes the
release's audit bundle (\texttt{release\_audit/}: the five files of
the reproducibility contract plus the per-candidate kill index and
the chirality audit, under a SHA-256 certificate).  An interactive
Colab walkthrough with waterfall visualisation is available at
\url{https://colab.research.google.com/drive/1otO1Qwp_Lr3OsK6ykkeMRATanecPkKg6}.
Running \texttt{python fermion\_scan\_standalone.py} performs the following:
\begin{enumerate}[nosep,label=(\arabic*)]
  \item Enumerates all 1{,}680 distinct templates using
  \texttt{combinations\_with\_replacement}.
  \item Applies the seven filters in exact rational arithmetic with
  F6 in its CANONICAL full-system non-degeneracy form, deciding the
  complete anomaly system over the rationals (nullspace and
  rational-root machinery; zero-extension certificates for the
  higher-nullity survivors).
  \item Prints the canonical waterfall
  ($1680 \to 348 \to 324 \to 24 \to 12 \to 10 \to 5 \to 1$) and
  the F6 predicate triple on the twelve F1--F5 survivors (canonical
  $10/5$; spectator reduction $8/4$; uniform dimension $4/2$) with
  winner invariance asserted.
  \item Lists all 10 canonical post-F6 survivors with their exact
  rational witnesses, and the 5 post-CPT classes; the
  spectator-reduction listings ($8$ and $4$) are reported as the
  robustness form.
  \item Records the near misses: the 42-DOF stripped generation
  ($Y_Q = 0$ forced; the sharpest near-miss) and the non-minimal
  survivors.
  \item Verifies the scan-boundary arithmetic for P1--P5: P3 in its
  full F2$+$F5 color-dimension case-split form (dimensions 3, 6, 8;
  ported from the engine battery at commit
  \codeid{ff97524e002dcbb4101f5934cde992b5721788f7}, with the
  historical dominance comparison retained as a rider), and P4 in its
  class-dominance form.
  ...
  \item Runs 6 built-in red-team challenges: (RT1) combinatorial
  cross-check of the enumeration count, (RT2) octet handling under
  the canonical predicate with the forced charged-octet relation
  $Y_8 = -(3/5)\,Y_Q$ on the 66-DOF pair, (RT3) dead-code
  reachability audit, (RT4) the two-colored-doublet class-dominance
  mini-enumeration (class minimum 54 Weyl DOF, witness-verified;
  Proposition~\ref{prop:P4}), (RT5) the true-chirality audit
  statistic per canonical survivor (the conjugate-pair criterion;
  reported, never consumed as a filter), and (RT6) filter-order
  invariance (all $7! = 5{,}040$ orderings under the total
  spectator-reduction predicate).
  \item Asserts the final verification conditions, including the
  predicate triple and winner invariance.
\end{enumerate}
The script is designed so that any reader can reproduce the finite
scan, exact anomaly arithmetic, and numerical argmin on a standard
laptop in seconds, with no knowledge of the APF codebase required.  The Zenodo record archives
the v2 implementation (spectator-reduction predicate); the shipped
canonical script is the version-locked release's.

The finite tested-list arithmetic of the Lie-algebra classification (\S\ref{sec:gauge_class_supp}) is checked by
\codeid{check\_L\_gauge\_template\_uniqueness}, which tests the
...
fields; P3's F2$+$F5 joint exclusion for colorless triplets):
...
\item \textbf{Multiplicity caps (declared inputs; P3 budget exclusion; P4--P5 dominance).}\quad
...
(\S\ref{sec:exclusion_proofs}).  P3 excludes colorless triplets a priori
(F2$+$F5); P5 is a dominance result: within the declared space, every
survivor carrying types beyond the five SM roles extends an
anomaly-closed core and sits strictly above it in DOF, so the
enumerated enlargement directions cannot lower the
minimum.
...
Weyl-fermion template within the declared representation bounds and
...
contract (\S\ref{sec:fermion_scan}), carried by P3's budget exclusion and P5-style dominance
...
produces the same final set.  The standalone script's RT6 sweep evaluates all
$7!=5{,}040$ orderings using the total spectator-reduction F6 variant,
because that predicate is total on the full 1{,}680-template universe.
Every ordering gives the same four post-CPT classes and 45-component
minimum.  The mathematical conjunction proof is predicate-independent
and therefore also covers the canonical full-system survivor set.
...
CPT pairs (listed in \S\ref{sec:waterfall}); the spectator-reduction
robustness variant (reported by the standalone script alongside its
canonical full-system predicate) yields 8, forming 4 pairs.

\item \textbf{Sensitivity to generation count.}\quad The scan assumes
$N_{\mathrm{gen}} = 3$ (imported at its banked ground, Paper~2 \S9).  With
...
\bibitem{Hughes1980}
R.\,J.~Hughes, ``Some comments on asymptotic freedom,''
\textit{Phys.\ Lett.\ B} \textbf{97}, 246--248 (1980).

\bibitem{Nielsen1981}
N.\,K.~Nielsen, ``Asymptotic freedom as a spin effect,''
\textit{Am.\ J.\ Phys.} \textbf{49}, 1171--1178 (1981).

\bibitem{BrookeFermionScan}
E.\,S.~Brooke, ``APF Fermion Template Scan---Standalone Verification
Script,'' v4.3, 2026 (the Zenodo record archives v2).
