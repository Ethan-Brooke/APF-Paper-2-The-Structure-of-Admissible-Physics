...
\begin{abstract}
\noindent
Paper~1: \emph{The Enforceability of Distinction} derived the quantum skeleton---Hilbert space, the Born rule, gauge symmetry, tensor products, and entropy---from the four structurally necessary components of the \emph{Principle of Least Enforcement Cost} (PLEC): A1 (finite enforcement capacity --- upper bound on admissibility), MD (uniform per-test cost floor --- positive lower bound), A2 (minimum cost --- argmin selection), BW (budget-window richness --- non-degeneracy), together with constitutive admissibility semantics (SP, K3, FD1--FD6).  PLEC is the canonical variational formulation of the enforceability-of-distinction foundation; it does not replace that foundation.  This paper deploys those results.

We begin with the Non-Closure Theorem ($L_{\mathrm{nc}}$): individually admissible enforcement demands, when composed at shared interfaces, generically exceed local budgets.  Admissible sets are not closed under composition.  This is the engine behind all competition, selection, and structure in the framework.  We provide constructive witnesses and quantitative consequences, then show that non-closure \emph{requires} non-abelian gauge structure: abelian composition is additive ($\Delta = 0$), which is incompatible with non-closure; the complete exclusion of abelian carriers is proved in the Technical Supplement~\cite{BrookeP2Supplement} ($B1'$, \S3.3), and the structural argument establishing that only non-abelian routing can sustain $\Delta > 0$ is developed in Section~\ref{sec:gauge_origin}.
...
The gauge template is then proved unique by exhaustive classification of all 17 compact simple Lie algebras: the three carrier requirements derived in Paper~1~\cite{BrookePaper1} (complex ternary, pseudoreal binary, abelian grading) select $\SU(N_c) \times \SU(2) \times \U(1)$ as the only viable product structure.  Capacity minimality selects $N_c = 3$.  The fermion content is derived by exhaustive scan of 1{,}680 chiral templates under seven filters, with five closed-form exclusion proofs covering all representations outside the scan.  The unique survivor is the Standard Model at 45 Weyl fermions.
...
The precise identification of which channels carry spin enforcement and which carry position enforcement depends on spacetime structure that this paper does not yet derive.  That accounting is completed in Paper~5: \emph{Spacetime and Gravity}, where the local Lorentz frame channels are named and the hydrogen atom example can be given its full channel-level treatment.  What this paper establishes is the mechanism: both tasks draw from the same local enforcement pool, their joint cost exceeds the sum of their individual costs, and the consequence is structural.  The starting point is the arena established by Paper~1: the finite orthogonal sector form ($T_{\mathrm{sector}}$, Technical Supplement~\cite{BrookeSupplement}), which packages the substrate into a finite-dimensional vector space $V$ with block-orthogonal inner product, sector decomposition $V = (\bigoplus_d M_d) \oplus \Pi$, and self-adjoint enforcement projections.  Paper~2 builds on this arena.
...
This is \emph{non-closure}: the set of individually admissible enforcement configurations is not closed under combination.  Putting it plainly: nature can afford A, and nature can afford B, but nature cannot always afford A and B at the same time.  Non-closure was proved in Paper~1~\cite{BrookePaper1} (\S4.3); this paper explores its consequences.
...
Section~\ref{sec:gauge_template} is the main event.  The three carrier requirements meet the complete classification of compact simple Lie algebras---all 17 of them---and every candidate except one is eliminated.  $\SU(3) \times \SU(2) \times \U(1)$ is not selected from a shortlist; it is the only structure that survives.  An exhaustive scan of 1{,}680 chiral fermion templates under seven simultaneous filters then yields a single survivor: the Standard Model at 45 Weyl fermions.  A final subsection derives the scalar sector: one complex $\SU(2)$ doublet, four real components, derived from the chiral fermion content and A2 (minimum cost).
...
Paper~2 takes Paper~1's quantum skeleton (A1 + PLEC $\rightarrow$ finite-dimensional complex Hilbert space) as input and derives the gauge group, fermion content, and cosmological fractions.  Five structural primitives carry the argument.  This box is a wayfinder, not new content; every claim points to the section that defines or formalizes the term.
...
\begin{description}[topsep=2pt,itemsep=2pt]
  ...
  \item[Fermion template.]  The 45-fermion Standard Model content as the unique admissible survivor of 4,680 gauge-compatible candidates under anomaly cancellation, Witten parity, and capacity budget.  \emph{Enumerated:} \S\ref{sec:gauge_template} (three-stage argument + exhaustive derivation).  \emph{Hypercharges derived:} \S\ref{sec:gauge_template}.
\end{description}
...
\smallskip\noindent\textbf{Derivation.}  Established in Paper~1 via the bridge chain $L_\Delta \to L_{\mathrm{blk}} \to T_{\mathrm{alg}}$ (full proof: Technical Supplement~\cite{BrookeSupplement}, \S3--4).  The pre-algebraic route derives $\Delta > 0$ from K2, K3, $T_{\mathrm{sep}}$, and SP (non-null directions in the pool $\Pi$) without invoking Hilbert space or field selection.  The key step: coordinating two distinctions at a shared interface requires defending both against pool-supported perturbations---a third enforcement act beyond the two individual ones, whose cost is $\Delta > 0$.  The joint defender's codespace $W_*$ is rotated into $\Pi$ ($L_\Pi$), producing the off-diagonal component $F_\Pi \ne 0$ that makes the enforcement algebra noncommutative ($T_{\mathrm{alg}}$).  Formally: the automorphism group of the local operator algebra is non-trivial (T3), so the enforcement map is order-sensitive, and the marginal cost $m(d_2 \mid d_1) = \varepsilon(\{d_1,d_2\}) - \varepsilon(d_1) \ne \varepsilon(d_2)$.
...
This is the operational origin of noncommutativity (T1 in Paper~1~\cite{BrookePaper1}): the order in which enforcement commitments are made affects the total cost.  Enforcing spin-up-vs-down \emph{first}, then position-here-vs-there, costs a different amount than the reverse order.
...
The connection between non-closure and gauge structure is the central structural argument of the framework.  The logical chain runs: $L_\Delta$ (superadditivity, from K2 + K3 + $T_{\mathrm{sep}}$ + SP) derives $\Delta > 0$ at any interface with a non-trivial pool.  $L_{\mathrm{blk}}$ shows that direct-sum encoding of jointly enforced distinctions is impossible when $\Delta > 0$.  The joint defender's codespace rotates into the pool ($L_\Pi$), producing off-diagonal generators $F_\Pi \ne 0$ that make the enforcement algebra noncommutative ($T_{\mathrm{alg}}$: $[E_{d_1}, F_\Pi] \ne 0$).  The Hilbert space is then constructed from the algebra via GNS ($T_{\mathrm{GNS}}$), not assumed beforehand.  Non-closure is not a consequence of non-commutativity; non-commutativity is a consequence of non-closure.  The Technical Supplement~\cite{BrookeSupplement} provides the complete derivation; the introduction sketched the intuition; we now trace the consequences for gauge structure.
...
An abelian gauge group---a product of $\U(1)$ factors, where $\U(1)$ is the group of phase rotations---routes enforcement additively.  Each $\U(1)$ channel contributes independently; costs simply add.  This is the additive countermodel of Paper~1~\cite{BrookePaper1} (\S4.3): $\Delta = 0$ implies full reversibility and no non-closure.
...
\textbf{Statement.}  The local enforcement algebra acts irreducibly on the GNS Hilbert space $\mathcal{H}_\omega$ (Theorem~$T_{\mathrm{GNS}}$, Technical Supplement~\cite{BrookeSupplement}).  That is, $\mathcal{H}_\omega$ has no proper closed subspace that is invariant under every enforcement operator.

\smallskip\noindent\textbf{Proof.}  Suppose for contradiction that a proper closed invariant subspace $\mathcal{V} \subsetneq \mathcal{H}_\omega$ exists.  Every state in $\mathcal{V}$ and every state in $\mathcal{H}_\omega \setminus \mathcal{V}$ are mapped, under every enforcement operator, into their respective subspaces.  This means the enforcement algebra cannot produce any operator whose action distinguishes a state in $\mathcal{V}$ from a state outside it.  Equivalently: two states, one from $\mathcal{V}$ and one from $\mathcal{H}_\omega \setminus \mathcal{V}$ that agree on all operators within $\mathcal{V}$, are assigned identical enforcement records.  But A1 requires that all physically distinct admissible states receive distinguishable enforcement records.  These two states are distinct (they lie in different subspaces) yet receive the same record: a direct contradiction.  Therefore no proper invariant subspace exists; the algebra acts irreducibly.  $\square$
...
\textit{Step 1 ($L_\Delta$: superadditivity).}  For co-located distinctions $d_1, d_2$ at an interface $\Gamma$ with non-trivial pool $\Pi \ne \{0\}$, pool-supported perturbations threaten both distinctions simultaneously.  K2 + K3 + SP give $\Delta(d_1, d_2) > 0$: the joint enforcement cost strictly exceeds the sum.
...
\smallskip\noindent\textbf{Corollary (Schur route).}  Once the algebra is represented on $\mathcal{H}_\omega$ (via $T_{\mathrm{GNS}}$), $L_{\mathrm{irred}}$ confirms irreducibility, and Schur's lemma gives the familiar characterization: any operator commuting with the entire irreducible algebra must be a scalar.  Since enforcement operators are non-trivial, non-commutativity follows independently.
...
\smallskip\noindent\textit{Distinction from classical contention.}  A classical resource-contention system can produce $\varepsilon(\{d_1,d_2\}) > \varepsilon(d_1) + \varepsilon(d_2)$, but with a symmetric surplus $\Delta_{12} = \Delta_{21}$.  That matches $L_{\mathrm{nc}}^{\mathrm{budget}}$ (classical pigeonhole).  The pre-Hilbert route derives asymmetry ($\Delta_{12} \ne \Delta_{21}$) directly from codespace rotation ($L_\Pi$): the rotation angle $\theta$ generically differs depending on enforcement order.  Witness: $\sigma_x \sigma_z = -\sigma_z \sigma_x$ (\texttt{check\_T0}, core.py).

\coderef{check\_T0, check\_L\_nc}{core.py}
\end{tcolorbox}

Formally, T3 (Paper~1) then establishes that the automorphism group of the non-commutative local enforcement algebra is the gauge group.  If the algebra were abelian, every automorphism would be trivial (phase rotations only), and $\Delta = 0$ identically.  Non-closure requires a non-trivial gauge group with non-commuting generators.

\smallskip\noindent\textit{Gauge identification (detailed in Technical Supplement~\cite{BrookeP2Supplement}, \S2).}\quad The Technical Supplement proves the full gauge identification theorem: the gauge group is the inner automorphism group $G = \mathrm{Inn}(\mathcal{A}) = \prod_k \mathrm{PU}(n_k)$, sitting in the exact sequence $1 \to Z(\mathcal{A}) \to \mathcal{U}(\mathcal{A}) \to \mathrm{Inn}(\mathcal{A}) \to 1$.  Abelian factors arise exclusively from the center $Z(\mathcal{A})$ and carry no self-interaction ($\Delta = 0$ within the abelian sector).  For the Standard Model template, the lift from $\mathrm{PU}(n)$ to $\SU(n)$ and the global structure $[\SU(3) \times \SU(2) \times \U(1)_Y]/\mathbb{Z}_6$ are established as derived consequences.

\smallskip
\noindent\textbf{The precise non-abelian criterion.}\quad Interference non-closure ($L_{\mathrm{nc}}^{\mathrm{interf}}$) is not merely superadditivity ($\Delta > 0$) but \emph{order-asymmetric} superadditivity ($\Delta_{12} \ne \Delta_{21}$).  A classical congestion system can have $\Delta > 0$ with $\Delta_{12} = \Delta_{21}$ (the contention cost is symmetric in the order of operations).  Only when $\Delta_{12} \ne \Delta_{21}$---which $L_{\Delta\to\mathrm{nc}}$ derives from codespace rotation ($L_\Pi$) in the enforcement algebra---does the algebra become genuinely non-commutative.  It is therefore order-asymmetric superadditivity, not mere superadditivity, that requires non-abelian gauge structure.

\coderef{check\_T3}{core.py}

\smallskip
\noindent\textbf{Proof boundary.}\quad The argument above establishes that the gauge group must be non-trivial and non-commutative.  A complete proof that \emph{no} abelian routing can sustain $\Delta > 0$ requires Lemma $B1'$ (Technical Supplement~\cite{BrookeP2Supplement}, \S3.3), which shows that any complex carrier admitting only bilinear invariants produces composites satisfying $B = \bar{B}$---the composite is self-conjugate and provides no independent enforcement channel under $T_M$.  The ternary invariant and complex carrier required by R1 are the \emph{minimal} structure that survives this constraint.  Paper~2 establishes the structural necessity of non-abelian routing via the oriented-composite argument; $B1'$ closes the proof that bilinear (abelian-type) carriers are generically excluded.
...
This is $T_{\mathrm{confinement}}$: confinement is derived from A1 $+$ asymptotic freedom $+$ finite capacity.  The structural necessity of the $\beta$-function sign is established in the Technical Supplement~\cite{BrookeP2Supplement}, \S4; the quantitative one-loop coefficient is derived in Paper~4A.  The mechanism is not imported from lattice QCD or any other external theory.  It follows directly from the cascade:
\begin{center}
\small
...
\medskip
\textbf{The irreversibility problem.}  Non-closure requires irreversibility ($L_{\mathrm{irr}}$, derived in Paper~1, \S4.5): some enforcement commitments cannot be undone.  But enforcement independence ($T_M$) requires that this irreversibility be \emph{intrinsic} to the gauge sector---each gauge factor must provide its own irreversible channels, not merely inherit irreversibility from gravity or from the environment.  This is a stronger requirement than simply producing entropy: a vector-like gauge theory coupled to an environment can produce entropy through decoherence without the gauge sector itself containing any intrinsically irreversible process.  The requirement is that the gauge structure, in isolation, breaks time-reversal symmetry.
...
\item \textit{Source}: $L_{\mathrm{nc}}$ (non-closure requires stable composites) $+$ $T_{\mathrm{confinement}}$ (IR saturation makes non-singlets inadmissible; schematic $\beta$-function sign: Technical Supplement~\cite{BrookeP2Supplement}, \S4) $+$ $T_M$ (enforcement independence requires oriented composites) $+$ $B_1'$ (complex type blocks $B \leftrightarrow \bar{B}$ identification; Technical Supplement~\cite{BrookeP2Supplement}, \S3.3).
\end{varlist}
...
\item \textit{Source}: enforcement completeness (A1 requires that the gauge structure distinguish all physically distinct matter representations) $+$ A2 minimality (one $\U(1)$ resolves all degeneracies; more than one is non-minimal).
\end{varlist}
...
\begin{varlist}
\item C1 $\leftarrow$ $L_{\mathrm{loc}}$ + $L_{\mathrm{cost}}$: independently enforceable channels at separate interfaces add costs (locality requires additive decomposition over interfaces).
...
\medskip\noindent\textit{Step 6 (No simple-group alternative).}  Any simple group $G_{\mathrm{simple}}$ containing $\SU(3) \times \SU(2) \times \U(1)$ has $\dim(G_{\mathrm{simple}}) \ge 24 > 12 = \dim(\SU(3) \times \SU(2) \times \U(1))$.  The product structure is always cheaper.  Independence of enforcement roles (Step~1) also requires factorization.  $\square$
...
\textit{Phase 1 (Scan).}  1{,}680 distinct chiral templates constructed from $\SU(3)$ representations $\{3, \bar{3}, 6, \bar{6}, 8\}$ crossed with $\SU(2)$ representations $\{1, 2\}$, up to 5 field types.  Seven sequential filters applied: (1)~$\SU(3)$ asymptotic freedom, (2)~$\SU(2)$ asymptotic freedom, (3)~chirality, (4)~$[\SU(3)]^3$ anomaly cancellation, (5)~Witten anomaly freedom~\cite{Witten1982}, (6)~full anomaly system with rational hypercharge solutions (including spectator-field reduction for $A(R)=0$ representations), (7)~CPT quotient.  Waterfall: $1{,}680 \to 348 \to 324 \to 24 \to 12 \to 8 \to 4$.  Minimality selects the unique survivor at 45 Weyl DOF.
...
\item P4: Multi-colored-multiplet templates have DOF $\ge 63 > 45$ (minimality).
...
\smallskip\noindent\textit{Why gauge constraints are not subtracted (the BRST question).}\quad A field-theorist's instinct is to subtract gauge redundancies via BRST ghost counting, reducing the number of physical degrees of freedom below $\dim(G)$.  The admissibility framework does not subtract because the counting principle is different: what A1 constrains is the number of \emph{independently enforceable structural distinctions}, not the number of propagating modes.  Each Lie-algebra direction is a structural enforcement channel that the interface must independently track, regardless of whether its kinematic content propagates as a physical particle or is removed by gauge-fixing.  The Technical Supplement~\cite{BrookeP2Supplement}, \S III.1.2 provides the full argument.
...
The QCD vacuum angle $\theta$ parameterises a topological term $\mathcal{L}_\theta = (\theta/32\pi^2)\,G\tilde{G}$ that is a total divergence.  It introduces no new propagating degrees of freedom and leaves $C_{\mathrm{total}} = 61$ unchanged.  However, maintaining a definite value $\theta = \theta_0 \ne 0$ is an enforceable distinction: distinguishing $\theta_0$ from other values in $[0, 2\pi)$ costs at least $\varepsilon^* > 0$ by $L_{\varepsilon^*}$.  The value $\theta = 0$ is unique: it is the only value at which the strong sector is CP-invariant, so $\theta = 0$ is not an independent parameter but a consequence of the symmetry.  No enforcement record is needed to maintain a symmetry consequence; the cost is zero.  Both $\theta = 0$ and $\theta \ne 0$ are admissible under A1: each respects $\Sigma\varepsilon \le C$.  But $\theta = 0$ is admissible at zero maintenance cost (the symmetry consequence carries no enforcement record), while $\theta \ne 0$ is admissible only at $\varepsilon^* > 0$.  Since the two configurations realise the same structural content but at strictly different total cost, A2 (minimum cost) selects $\theta_{\mathrm{QCD}} = 0$ as the realised value over the admissible set.
...
The enforcement potential $V(\Phi)$ derived from $L_{\varepsilon^*}$, $T_M$ (monogamy binding), and A1 (capacity saturation) has a unique global minimum: $V(0) = 0$ (empty vacuum, unstable), a barrier at $\Phi/C \sim 0.06$, a unique binding well at $\Phi/C \sim 0.73$ with $V < 0$, and a divergence at $\Phi \to C$ (capacity saturation).  The divergence at $\Phi \to C$ prevents any second minimum from forming at high field values.  The electroweak vacuum occupies the unique well and is absolutely stable---not metastable with a long lifetime, but stable as a matter of enforcement structure.
...
\item \textbf{Spacetime derivation.}  The Technical Supplement~\cite{BrookeP2Supplement} derives Lorentzian signature ($\Delta_{\mathrm{sig}}$), spacetime dimension $d = 4$ ($T_8$), and the Einstein field equations ($T_{9,\mathrm{grav}}$) from the cost kernel and Lovelock's theorem.  These are conditional on the full dynamical treatment in Paper~6 but are self-contained at the structural level.
\item \textbf{Neutrino masses.}  The 45-fermion prediction (Majorana neutrinos, no right-handed neutrino in the gauge spectrum) is a sharp falsifiability test: the $\nu_R$-extended template (48 Weyl fermions) predicts $C_{\mathrm{total}} = 64$ and $\Omega_\Lambda = 42/64 = 0.656$, excluded at ${>}\,5\sigma$ by Planck~\cite{Planck2020}.  The full analysis is in the Technical Supplement~\cite{BrookeP2Supplement}, \S9.
\end{varlist}
...
\item \textbf{(F7)} Dirac neutrinos with standard electroweak couplings (changes $C_{\mathrm{total}}$ from 61 to 64, predicting $\Omega_\Lambda = 42/64 = 0.656$---excluded at ${>}\,5\sigma$ by Planck~\cite{Planck2020}).  The 45-fermion vs.\ 48-fermion discrimination is the sharpest single-observable test of $T_{\mathrm{field}}$; see Technical Supplement~\cite{BrookeP2Supplement}, \S9.
...
Gauge template & $L_{\mathrm{gauge,uniq}}$ & 17 algebras $\to$ unique & \texttt{check\_L\_gauge\_template\_uniqueness} \\
...
Vacuum stability & $T_{\mathrm{vac,stab}}$ & Unique enforcement well & \texttt{check\_T\_vacuum\_stability} \\
Proton stability & $T_{\mathrm{proton}}$ & Exact $B$ conservation at saturation & \texttt{check\_T\_proton} \\
\bottomrule
\end{tabular}
\end{center}

Free parameters introduced: 0.  Physics imported: 0.  The Standard Model's gauge group, matter content, and three structural predictions (strong CP, vacuum stability, proton stability) are derived from a capacity optimization problem stated in the language of A1.
...
\begin{center}
\footnotesize
\begin{tabular}{lp{3.2cm}p{4.5cm}l}
\toprule
...
All check functions reside in \texttt{core.py}, \texttt{gauge.py}, \texttt{cosmology.py}, \texttt{gravity.py}, or \texttt{L\_Cauchy\_uniqueness.py}.  The fermion-content derivation ($T_{\mathrm{field}}$) additionally has a standalone verification script \texttt{fermion\_scan\_standalone.py}~\cite{BrookeFermionScan} with an interactive Colab walkthrough.  ``P1S'' denotes the Paper~1 Technical Supplement~\cite{BrookeSupplement}; ``P2S'' denotes the Paper~2 Technical Supplement~\cite{BrookeP2Supplement}.
...
A1 & Finite enforcement capacity (first axiom) & P1, \S2 \\
A2 & Minimum cost (second axiom) & P1, \S2 \\
...
$T_{\mathrm{alg}}$ & Noncommutativity of enforcement algebra & P1S, \S4 \\
...
$\varepsilon^*$ & Minimum enforcement cost & P1, \S4.1; P1S, $L_{\varepsilon^*}$ \\
$\kappa$ & Directed multiplier ($= 2$) & P1, \S5.11 \\[4pt]
\multicolumn{3}{l}{\textit{Introduced in this paper (proofs: Technical Supplement~\cite{BrookeP2Supplement})}} \\[2pt]
...
$T_{\mathrm{vac,stab}}$ & Vacuum absolute stability (unique enforcement well) & \S\ref{sec:vacuum_stability} \\
\bottomrule
\end{tabular}
\end{center}
...
\bibitem{BrookeSupplement} E.~S.~Brooke, ``Technical Supplement: End-to-End Derivation from Finite Enforcement Capacity to Quantum Structure,'' companion to~\cite{BrookePaper1}.  Zenodo: 10.5281/zenodo.XXXXXXX.

\bibitem{BrookeP2Supplement} E.~S.~Brooke, ``Technical Supplement: The Structure of Admissible Physics---End-to-End Derivations,'' companion to this paper, 2026.  Self-contained proofs of $L_{\mathrm{Cauchy}}$, gauge identification, $T_{7B}$, $L_{\mathrm{irr,uniform}}$, $B1'$, $L_{\mathrm{gauge,unique}}$, $T_{\mathrm{field}}$, $L_{\mathrm{hypercharge}}$, $L_{\mathrm{count}}$, $P_{\mathrm{exhaust}}$, $L_{\mathrm{equip}}$, $L_{\mathrm{sat,part}}$, spacetime derivation chain ($\Delta_{\mathrm{sig}}$, $T_8$, $T_{9,\mathrm{grav}}$).  All imported Paper~1 results re-proved in Appendix~C.
