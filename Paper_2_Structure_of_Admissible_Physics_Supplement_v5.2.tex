...
doublet.  Suppose exactly one.  In the $\SU(2)$ asymptotic-freedom
ledger, each entry contributes
$(2/3)\,T(r^{(2)})\dim(r^{(3)})\,N_{\mathrm{gen}}$: the colored
doublet contributes $3$, and a colorless triplet ($T(\mathbf{3}) = 2$)
contributes $4$, leaving $22/3 - 7 = 1/3$ of the budget.  But one
colored doublet alone has odd Witten parity ($3$ doublets counted with
color multiplicity), and any parity-restoring addition costs at least
a further $1$ (a colorless doublet; larger half-integer
representations cost more), for a total of at least
$3 + 4 + 1 = 8 > 22/3$: F2 fails.  Hence no one-colored-doublet
template containing a colorless triplet satisfies F2 and F5 jointly.
Templates with two or more colored doublets are dominated by
Proposition~\ref{prop:P4} (class minimum 54); templates with no
...
\begin{enumerate}[label=\textbf{F\arabic*},nosep,leftmargin=3em]
  ...
  \item \textbf{Full anomaly system non-degenerately solvable.}\quad
...
\begin{enumerate}[label=(C\arabic*),nosep]
  ...
  (F4 and F6, and F5, respectively), consumed as filter conditions; their solutions are
...
supplied by H3, not derived; the CM leg (no second factor) is [P]
...
unique ray, realized in $T_{\mathrm{rel}}$ by H3's action clause.}\quad
...
relabeling --- an unconditional computed fact; the corresponding
direction $\mathbf{q}$ in $T_{\mathrm{rel}}$ exists and is unique
given H3's action clause (the ray lies in the image of $c$, and $c$
separates the types).  (``Admits'' rather than ``selects'': no
...
capacity (A1), this is inadmissible.  (Generation universality is
...
$T_{\mathrm{rel}}$ by H3's action clause; and no second factor is
...
Script,'' v4.1, 2026 (the Zenodo record archives v2).
