...
- The [executable codebase](apf/) (run `python verify_all.py` for the full 20-check verification; `--paper2-core` is the fast path);
- The [Colab notebook](APF_Reviewer_Walkthrough.ipynb) (zero-install, theorem-by-theorem cells with prose + code);
- The [interactive derivation DAG](https://ethan-brooke.github.io/APF-Paper-2-The-Structure-of-Admissible-Physics/) (hover-for-details, click-for-dependencies, animated verification).

---

## The derivation in 14 steps

*(The 14 steps below walk the paper's spine. Six further checks ride alongside them in this release — see "The classification core, verified" at the end of this guide.)*
...
- **Multiplicity-cap completeness** of the 1,680-template scan is OPEN: the caps are declared inputs, and uniqueness is within the enumerated class (candidates outside the caps are walled off only by the named exclusion proofs P1–P5).
- **The foundational premises** — operational encoding and disjoint-anchor realizability (for C(G) = dim G), the EC/CM reading, A2 argmin selection, N_gen = 3 — are named inputs consumed at their banked grounds; their derivations are the object of Technical Supplement II's graded program, not of this paper.
...
Execution model: `run_checks.py` calls `bank.run_all()`, which iterates over every registered check and aggregates pass/fail/error counts. Individual checks can be invoked via `apf.bank.get_check('check_T_field')()`.
...
*Generated by the APF `create-repo` skill on 2026-04-19; refreshed through the v5.x review cycle. Codebase at this release: v24.3.423 (numerical kernel 5bc6193; bank 3,912 entries, native audit 3912/3912 gap 0; 20 checks bundled in this repo).*

---

## The classification core, verified (added at the v5.1-release refresh)

Six checks beyond the 14-step spine carry the review-cycle's classification core:

- **`check_L_cost_gauge`** — C(G) = dim(G)·ε under the operational-encoding and disjoint-anchor-realizability premises, named in the statement.
- **`check_T_sep` / `check_L_loc`** — the separability and locality inputs the carrier argument consumes.
- **`check_T_field`** — the ordered-tuple scan (uniform-dimension F6 path) plus the five exclusion proofs, with P4 executed as the corrected 54-witness class-dominance battery (no tightness claim).
- **`check_L_F6_not_from_EC`** — the canonical full-system waterfall (10 pre-CPT / 5 post-CPT) and the computed logical independence of EC-label-injectivity and F6 (all four quadrants witnessed).
- **`check_L_EC_inventory_reading`** — the EC inventory reading at its honest grade (A1-motivated, not A1-derived).

The scan itself has ONE canonical executable: `python fermion_scan_standalone.py --emit-audit` regenerates every waterfall count and the release audit bundle (`release_audit/`, six data files + SHA-256 certificate) in exact rational arithmetic, with red-team checks RT1–RT6 (RT6: filter-order invariance across all 5,040 orderings under the total spectator-reduction F6 predicate; the conjunction theorem is predicate-independent). Anticipated objections and withdrawn claims are catalogued in `ai_context/CLAIMS_LEDGER.md` and `ai_context/DO_NOT_CLAIM.md`.
