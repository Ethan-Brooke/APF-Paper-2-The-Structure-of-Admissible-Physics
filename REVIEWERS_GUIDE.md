...
The scan itself has ONE canonical executable: `python fermion_scan_standalone.py --emit-audit` regenerates every waterfall count and the release audit bundle (`release_audit/`, six data files + SHA-256 certificate) in exact rational arithmetic. Anticipated objections and withdrawn claims are catalogued in `ai_context/CLAIMS_LEDGER.md` and `ai_context/DO_NOT_CLAIM.md`.
