...
independent phenomenological assumption), the Weyl-component objective, and the equal-unit channel
...
The unique minimal Weyl-fermion template consistent with
$\SU(3) \times \SU(2) \times \U(1)$, three generations,
the F6 non-degeneracy requirement ($Y_Q \neq 0$ --- an independent
assumption, declared at its point of use in
\S\ref{sec:seven_filters}, in its canonical full-system form), and all
...
class.  Then:
\begin{enumerate}[label=(\roman*),nosep]
  \item If $\delta = 0$, no abelian factor is needed.
  \item If $\delta > 0$ (equivalently $m_{\max} \ge 2$), the
  non-abelian labels leave some class unseparated, and at least one
  abelian factor is needed.
  \item If $\delta > 0$ and the specified relative-torus
  representation (H3's matter action) contains an anomaly-compatible
  character that is injective on every deficient class, then one
  effective $\U(1)$ suffices.
\end{enumerate}
Sufficiency is not from $\delta$ alone: a physical $\U(1)$ cannot
assign arbitrary unrelated real numbers to the matter types --- its
charges must arise from one character of the specified torus action,
under the compact character lattice, anomaly compatibility, and
generation universality (C3).  Clause~(iii)'s hypothesis packages
exactly those requirements.
\end{lemma}

\begin{proof}
(ii): some class $P_k$ with $|P_k| \ge 2$ carries identical
non-abelian labels; EC then requires a further label source, and by
source exclusivity (Step~1 of the consolidated theorem) that source is
abelian.  (iii): the hypothesized character is a single scalar
coordinate on $\mathcal{M}$, distinct within every deficient class,
so the augmented label map $r_i \mapsto ([r_i], Y_i)$ is injective;
one effective factor suffices, and by CM no more than one is
admissible once injectivity holds.
\end{proof}

\begin{lemma}[Diagonal grading; rational ratios on the selected template]
\label{lem:A3_derived}
Assume the compact-$\U(1)$/character-lattice premise (front-matter
imported-ingredient ledger) and fix the selected five-multiplet
template.  Then each $\U(1) \subset T_{\mathrm{rel}}$, acting through
H3's central characters, assigns a charge to each $r_i$ constant on
that multiplet type, and on the selected template the anomaly system
forces rational (indeed integer) charge ratios.  The scope matters:
the rationality derivation below consumes the selected template's
anomaly polynomial, not a general property of the torus; for a
general compact torus the correct general statement is integrality of
closed-subgroup characters relative to the declared character
lattice, with anomaly cancellation then restricting which integral
combinations are admissible.  The two properties in detail:
\begin{enumerate}[label=(\alph*),nosep]
...
beyond EC, CM, the selected gauge template, the scanned matter
content, the compact-$\U(1)$ character-lattice premise, and
I-typing's central-character action (H3) is needed.
...
\begin{enumerate}[label=(\roman*),nosep]
  ...
  abelian gauge factor, unique as an effective charge ray in the
  physical quotient $T_{\mathrm{phys}} = T_{\mathrm{rel}} /
  \ker\rho_{\mathcal{M}}$, up to overall normalization.
\end{enumerate}
...
$m_{\max} = 2$.  By Lemma~\ref{lem:min_abelian}(iii), one $\U(1)$ factor
suffices provided the specified torus action supplies an
anomaly-compatible character assigning distinct charges to $u^c$ and
$d^c$.
...
The surviving abelian factor lives in
...
conditions (\S\ref{sec:hypercharge}) determines a \emph{unique ray
in charge space}, up to normalization
($Y \mapsto \lambda Y$, $\lambda \in \mathbb{Q}^\times$): the
anomaly system has exactly two solutions $z = 1 \pm N_c$ (with
$N_c = 3$: $z = 4$ and $z = -2$), corresponding to the same ray under
the $u \leftrightarrow d$ relabeling
(Theorem~\ref{thm:hypercharge}).  Given H3's action clause the ray is
realized, and its uniqueness as a physical action is uniqueness of the
effective ray in $T_{\mathrm{phys}} = T_{\mathrm{rel}} /
\ker\rho_{\mathcal{M}}$ --- torus directions differing by the
matter-action kernel induce the same charges (the consolidated
theorem's Step~4).  This gives (iv).
\end{proof}
...
\begin{enumerate}[label=(H\arabic*),nosep]
  ...
  $T_{\mathrm{rel}} \cong \mathrm{U}(1)^2$ on the five types is
  itself input data --- a central-character matrix
  $c : \mathcal{M} \to \mathbb{Z}^2$ assigning each \emph{type} its
  pair of relative-phase charges, with $c(u^c) \neq c(d^c)$ and with
  the hypercharge ray in the image of $c$.  This clause is
  load-bearing and cannot be replaced by representation data: the
  type-canonical invariants of the nonabelian labels ($\SU(3)$
  $n$-ality and $\SU(2)$ isospin parity) coincide on $u^c$ and $d^c$,
  so any torus action factoring through those invariants gives
  $Y_u = Y_d$ on every direction --- the hypercharge ray does not lie
  in that image.  Antecedently distinct types (clause one) must
  therefore carry antecedently distinct central characters (clause
  two) for the closure below to go through; generation universality
  (C3 of Theorem~\ref{thm:conditional_classification}) is part of the
  same clause --- $c$ is a function of type, acting identically on the
  three generation copies.  One further definition fixes where the
  uniqueness claim lives: let $\rho_{\mathcal{M}}$ be the matter
  action defined by $c$, and
  $T_{\mathrm{phys}} := T_{\mathrm{rel}} / \ker\rho_{\mathcal{M}}$
  the \emph{physical quotient}.  Torus directions differing by
  $\ker\rho_{\mathcal{M}}$ induce the same charges on every
  multiplet, so no property of $c$ can distinguish them --- if $c$ has
  a kernel direction, entire families of one-dimensional subgroups of
  $T_{\mathrm{rel}}$ realize the same physical action.  The closure's
  uniqueness claim is therefore made for the \emph{effective charge
  ray in} $T_{\mathrm{phys}}$; the kernel itself is an inactive
  direction (zero admissibility information) and is removed by CM.
...
removed by CM.  This factor is hypercharge $\U(1)_Y$, with charge
...
that the ray lies in the image of the relative-torus action is
supplied by H3, not derived --- and uniqueness is of the effective
...
charge $Y_i$ to each $r_i$, constant on the type (Schur's lemma:
central elements act by scalars on irreducible representations), with
the assignment given by I-typing's central-character matrix (H3,
clause two); on the selected template the anomaly system makes the
charge ratios rational (\S\ref{sec:hypercharge}).  The
...
$(c_1, c_2)$ is I-typing's central-character matrix (H3, clause two).
...
Therefore the anomaly system admits a \emph{unique ray} in the
abstract five-charge space, up to normalization and the discrete
relabeling --- an unconditional computed fact.  Given H3's action
clause, a realizing direction $\mathbf{q}$ exists (the ray lies in
the image of $c$), and what is unique is its class in the physical
quotient: distinct directions of $T_{\mathrm{rel}}$ differing by
...
standard library) that reproduces the complete scan from scratch.  It
is THE canonical executable of the release: every count in the
waterfall table is generated by it, and
...
\begin{enumerate}[nosep,label=(\arabic*)]
  ...
  survivors.
  \item Verifies the five exclusion proofs (P1--P5) with explicit
  arithmetic, P4 in its class-dominance form.
...
fermion-content derivation on a standard laptop in seconds, with no
knowledge of the APF codebase required.  The Zenodo record archives
...
Script,'' v4.2, 2026 (the Zenodo record archives v2).
