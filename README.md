...
This is a **version-locked release**: the scan's numerical content is locked at the **numerical kernel commit `5bc6193`** of the canonical APF codebase (**v24.3.423**, 2026-07-14; 3,912 bank-registered entries, native `verify_all --bank-audit` 3912/3912, gap 0), with later engine commits limited to count-neutral proof-scope corrigenda (the trail is recorded in the release manifest, `release_audit/scan_inputs.json`); check functions are vendored at the corrigenda-trail head. Each theorem in the manuscript traces to a named `check_*` function in `apf/core.py` or `apf/ec_inventory_reading.py`, callable independently and returning a structured result.
...
The reproducibility contract in Technical Supplement I (v5.3) names five audit files a complete release must carry. They live in [`release_audit/`](release_audit/), generated by `python fermion_scan_standalone.py --emit-audit` at this release's version lock:

| File | Contents |
|---|---|
| `scan_inputs.json` | representation tables, multiplicity caps, filter definitions, equivalence relation, version lock (numerical kernel + corrigenda trail) |
...
├── apf/
│   ├── core.py                            ← 18 theorem check functions (numerical kernel 5bc6193; corrigenda-trail head)
...
├── fermion_scan_standalone.py             ← THE CANONICAL SCAN EXECUTABLE (v4.2; emits release_audit/)
├── docs/
...
├── Paper_2_Structure_of_Admissible_Physics_v7.2.tex                          ← the paper
├── Paper_2_Structure_of_Admissible_Physics_Supplement_v5.3.tex               ← Technical Supplement I — The Classification Core
