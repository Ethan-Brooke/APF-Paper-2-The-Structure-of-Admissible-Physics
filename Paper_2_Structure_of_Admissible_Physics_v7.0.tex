...
All results trace to named check functions in the codebase.  No continuous free parameters are introduced; the discrete premises of the classification (the scan caps, electromagnetic completeness, the admissibility-completeness reading) are named at their points of use.  The correlation space now has rooms, and we can count them.
...
Section~\ref{sec:gauge_template} is the main event.  The three carrier requirements meet the classification of compact simple Lie algebras---four infinite families and five exceptional cases, tested through a 17-representative list with the families closed in closed form---and every candidate except one is eliminated.  $\SU(3) \times \SU(2) \times \U(1)$ is not selected from a shortlist; it is the only structure that survives.  A complete scan of the 1{,}680 chiral fermion templates of the declared space under seven simultaneous filters then yields a single survivor: the Standard Model at 45 Weyl fermions.  A final subsection derives the scalar sector: one complex $\SU(2)$ doublet, four real components, derived from the chiral fermion content and A2 (minimum cost).
...
\begin{description}[topsep=2pt,itemsep=2pt]
  ...
  \item[Fermion template.]  The 45-fermion Standard Model content as the unique admissible survivor of 1{,}680 gauge-compatible candidates under anomaly cancellation, Witten parity, and capacity budget.  \emph{Enumerated:} \S\ref{sec:gauge_template} (three-stage argument + complete scan of the declared space).  \emph{Hypercharges derived:} \S\ref{sec:gauge_template}.
\end{description}
...
The scan is a complete enumeration of the declared space.  A \emph{chiral fermion template} is a finite multiset $\mathcal{T} = \{(r_i, n_i)\}$ where each $r_i$ is an irreducible representation of $\SU(3) \times \SU(2)$ and $n_i \ge 1$ is its multiplicity, subject to three constraints: (i) each $r_i$ is a left-handed Weyl field (the template convention; filter~3 below is a doublet--singlet content cut, and chirality of the winner is settled by the anomaly system's non-degeneracy), (ii) each $\SU(3)$ factor lies in $\{3, \bar{3}, 6, \bar{6}, 8\}$ (the AF bound P1 excludes larger representations), and (iii) each $\SU(2)$ factor is $\{1, 2\}$ (P2 excludes higher colored representations unconditionally; colorless triplets are held out by P3's dominance certificate).  The template size is bounded by the declared enumeration caps (one colored doublet, at most three colored singlets, one lepton doublet, at most two lepton singlets), so $|\mathcal{T}| \le 7$; P5 certifies that extra types cannot lower the minimum (dominance against the exhibited 45-DOF survivor --- the enumeration's own survivor list contains 6- and 7-entry templates at 48--72 DOF).  Under these constraints the search space is finite: 1{,}680 distinct candidate templates.\footnote{Colored singlets are unordered within a template (the multiset $\{u^c, d^c\}$ is one template, not two), so the correct enumeration uses combinations with replacement.  A standalone verification script reproducing the full scan with exact rational arithmetic, four built-in red-team challenges, and both hypercharge solutions is archived at \url{https://doi.org/10.5281/zenodo.19154197} with an interactive Colab walkthrough at \url{https://colab.research.google.com/drive/1otO1Qwp_Lr3OsK6ykkeMRATanecPkKg6}.  Source code: \url{https://github.com/Ethan-Brooke/APF-Paper-2-The-Structure-of-Admissible-Physics}.}  Five closed-form proofs (P1--P5 in the box below) bound the space: P1--P2 exclude the enumerated representation enlargements unconditionally, and P3--P5 certify dominance against the named extension directions.  The caps themselves are declared scan inputs, and their completeness is an open proof obligation of the reproducibility contract (Technical Supplement~I~\cite{BrookeP2Supplement}); the licensed claim is uniqueness of the minimum among the enumerated candidates.

Seven filters are then applied sequentially.  Each filter is an exact declared predicate, typed in Technical Supplement~I's input ledger (C1--C10): asymptotic freedom for both gauge factors (the coupling must weaken at short distances; the one-loop coefficient is imported), doublet--singlet content (colored doublets and singlets co-present --- a content cut; chirality of the winner is settled at the anomaly system's non-degeneracy), the anomaly cancellation conditions (the quantum theory must be mathematically self-consistent; imported equations, solved exactly, with electromagnetic completeness an independent assumption in its canonical full-system form), and capacity minimality (among the survivors, the cheapest wins).  The filters are not redundant---each eliminates candidates that passed all previous filters.

Of the 1{,}680 templates, 1{,}679 fail.  One survives.

\begin{tcolorbox}[apfbox, title={Theorem $T_{\mathrm{field}}$ (SM Fermion Content)}]
\textbf{Statement.}  The unique minimal chiral fermion content consistent with $\SU(3) \times \SU(2) \times \U(1)$, three generations, electromagnetic completeness (F6 --- an independent assumption, named at its point of use in the scan), and all remaining admissibility constraints is the Standard Model:
\begin{equation}
...
\textit{Phase 1 (Scan).}  1{,}680 distinct chiral templates constructed from $\SU(3)$ representations $\{3, \bar{3}, 6, \bar{6}, 8\}$ crossed with $\SU(2)$ representations $\{1, 2\}$, up to 7 field types.  Seven sequential filters applied: (1)~$\SU(3)$ asymptotic freedom, (2)~$\SU(2)$ asymptotic freedom, (3)~doublet--singlet content, (4)~$[\SU(3)]^3$ anomaly cancellation, (5)~Witten anomaly freedom~\cite{Witten1982}, (6)~full anomaly system with non-degenerate rational hypercharge solutions (F6, electromagnetic completeness --- an independent assumption, in its canonical full-system non-degeneracy form; spectator reduction and uniform dimension are the robustness variants), (7)~CPT quotient.  Waterfall: $1{,}680 \to 348 \to 324 \to 24 \to 12 \to 10 \to 5$ (post-F6 and post-CPT counts under the canonical full-system predicate, charged-octet solutions included; robustness: the spectator-reduction predicate gives $8 \to 4$ and the uniform-dimension code path $4 \to 2$ --- the minimality winner is identical under all three; Technical Supplement~I~\cite{BrookeP2Supplement}).  Minimality selects the unique survivor at 45 Weyl DOF.
...
\bibitem{BrookeP2Supplement} E.~S.~Brooke, ``Technical Supplement I --- The Classification Core: The Structure of Admissible Physics,'' companion to this paper, v5.1, 2026.  The conditional classification theorem with its full input ledger (C1--C10), $L_{\mathrm{gauge,unique}}$, $T_{\mathrm{field}}$ (the 1{,}680-template scan), $L_{\mathrm{hypercharge}}$, the one-U(1) a-posteriori closure theorems, $L_{\mathrm{count}}$, and the bridge-axiom ledger (BA1--BA10).
...
\bibitem{BrookeFermionScan} E.~S.~Brooke, ``APF Fermion Template Scan---Standalone Verification Script (v2),'' (2026).  Zenodo: \url{https://doi.org/10.5281/zenodo.19154197}.  GitHub: \url{https://github.com/Ethan-Brooke/APF-Paper-2-The-Structure-of-Admissible-Physics}.  Interactive walkthrough: \url{https://colab.research.google.com/drive/1otO1Qwp_Lr3OsK6ykkeMRATanecPkKg6}.
