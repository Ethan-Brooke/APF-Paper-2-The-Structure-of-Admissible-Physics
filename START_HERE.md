...
**What APF is.** APF is a single-axiom framework. The only axiom is that enforcement (the cost of maintaining physical distinction) is finite (Axiom A1). From A1 and its four-component structure (PLEC), a 3,912-entry theorem bank derives the Standard Model gauge group SU(3)xSU(2)xU(1), the fermion content, three generations, and the framework's quantitative predictions. Each paper contributes one layer: Paper 0 states the ontology; Papers 1-7 build the derivation chain; Paper 13 is the master reference. The codebase (v24.3.423) is the authoritative specification; the papers are prose guides to reading the code.

**What this paper contributes.** Non-closure theorem: the admissibility algebra admits no single representation valid across all admissibility regimes. The nonabelian carrier ranking SU(3)xSU(2) (one-U(1) closure proved a-posteriori for the selected matter content); the conditional 1,680-template classification theorem — relative to the declared representation list, caps, and filter inputs, the unique minimum-capacity survivor is the 45-Weyl-fermion SM template (canonical waterfall 1680 -> 348 -> 324 -> 24 -> 12 -> 10 -> 5 -> 1); capacity partition C_total = 61 = 45 + 4 + 12, read cosmologically as 42 + 19 (Omega_Lambda = 42/61, Omega_m = 19/61).

**What this repo verifies vs what it imports.** This repo locally verifies 20 theorem checks drawn from Paper 2's own dependency subset, vendored from canonical codebase v24.3.423 (numerical kernel commit 5bc6193; later engine commits are count-neutral corrigenda — the trail is in the release manifest). The scan's audit files in `release_audit/` are generated by the canonical executable `fermion_scan_standalone.py`. Results from Paper 1 are imported, not re-derived here. The full APF corpus (and which paper owns which result) is in §1 below.
...
- The current canonical state (v24.3.423, numerical kernel 5bc6193, bank 3912)
